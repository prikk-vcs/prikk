...
| **2** | **Commit memory — probe the object index in `prikk-store`** | Product | **RULED 2026-09-09 (§6d.1).** `IndexSnapshot::open` decodes the whole index into a `Vec<IndexEntry>` per write session — the only named candidate **known** to be held wholesale in RAM. `IndexEntry` is `pub(crate)`, so the probe belongs **inside `prikk-store`**, not `prikk-cli`; no visibility change needed. Needs a handoff |
