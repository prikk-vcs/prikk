...
| **1** | **RFC 144 increment 2 follow-ups — prove the queued fold site, un-stale increment 1's comment, make the batch fail-atomic** | Product | **RULED 2026-09-09 (§4k.1–§4k.3); blocks increment 3.** The routing at `apply_queued_patch_envelopes` is correct but unheld by any test, and that site is the unsealed-WAL path where multi-commit queuing lives — harmless only until authoring emits its first `RenamePath`. Handed off at `rfcs/handoffs/144-two-point-comparison/increment-2-followups-handoff-v1.md`
