...
| **1** | **RFC 131 §6c.7 — reconcile the allowlist and turn the gate green** | Product | **RULED 2026-09-10; handed off** at `rfcs/handoffs/131-module-grouping-and-visibility-scoping/allowlist-reconciliation-handoff-v1.md`. All 8 cycle entries stand — **one rename**, bare `lifecycle_cache` → `lifecycle_cache::replay`. Hubs settle by arithmetic (`min(fan_in, fan_out) >= 6`): remove `refs` (min 4, a **sink** not a middle hub) and `lifecycle_cache` (min 2); declare `active`, `wal`, `author::author_key_index`. `active`/`wal` **recover their prior reasons from git history** — they are a recorded artifact returning, not new debt. **The gate ends green.** §6a's lift is now available and is RFC 131's own next decision |
