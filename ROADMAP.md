...
| **1** | **RFC 131 §6c.5 — generalise subtree cycles from pairs to SCCs** | Product | **RULED 2026-09-10; handed off** at `rfcs/handoffs/131-module-grouping-and-visibility-scoping/subtree-scc-generalisation-handoff-v1.md`. Rule 2 is two-variable, so a cycle closing through a third node is inexpressible — `worktree_patch → patch_replay` has **no return edge**; it closes through `active`. **Third blindness in this arc, each found by fixing the last, each making the gate report fewer cycles** — §6c.6 rules a standing caution: a precision change that reduces findings is suspect until two vanished ones are hand-traced |
