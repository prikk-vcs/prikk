...
| **2** | **Commit memory — attribute the growth before optimising it** | Product | **RE-RULED 2026-09-09 (§6c.2).** Removing `seen_ids` is still justified as a **simplification** (provable redundancy) but **is no longer the memory remedy** — its position rested on a premise the measurement removed. Next step is **attribution**, handed off 2026-09-09 at `rfcs/handoffs/133-performance-cost-and-its-evidence/node-count-memory-attribution-handoff-v1.md`: `NodeLifecycleState`'s in-memory cost in isolation, `commit-index.v1`'s own growth, and a sweep for a third grower — **the deliverable is the residual**, not a list. `seen_ids` removal is sequenced *after* it: removing it changes `lifecycle-state.v1`'s size, one of the quantities being attributed |
