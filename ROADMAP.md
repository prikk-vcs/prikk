...
| **2** | **Commit memory — the object index's resident cost** | Product | **RULED 2026-09-09 (§6d.1), handed off** at `rfcs/handoffs/133-performance-cost-and-its-evidence/object-index-resident-cost-handoff-v1.md`. `ObjectReadSnapshot::open` / `ObjectWriteSession::open` each hold the whole index as a `Vec<IndexEntry>` — the only named candidate **known** to be resident, and never measured in the unit that matters. **§6d.1's first ruling was corrected the same day**: both types are `pub` and exported, so the probe needs **no visibility change and no `test-support` helper** — it runs from the existing instrument against the public API |
