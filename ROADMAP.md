...
| **1** | **RFC 139 increment 3** — checkout cost at depth, and merge-path baseline reconstruction | Product | **Proposed for this position, owner to authorize.** The instrument exists as of `329715b` and the round that built it is fresh; this is the payoff it was built for. **Bounded by the depth ruling**: at 256 blocks it can answer RFC 136 §9 item 1 (checkout at realistic depth) and **cannot** answer cadence tuning against `REANCHOR_BOUND` — four intervals show direction, not shape, and the increment must say so |
