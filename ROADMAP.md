...
| **2** | **Docs — the scale limits an adopter needs, and the gate set a contributor needs** | Product | **RULED 2026-09-09; handed off** at `rfcs/handoffs/133-performance-cost-and-its-evidence/docs-scale-limits-and-gate-set-currency-handoff-v1.md`. `reference/current-state.md`'s "Not a good fit yet" lists ten entries, **all feature gaps, none about scale** — while sealing is quadratic in depth and checkout/merge-evidence cost `depth^1.45`, so **cost tracks history depth, not repository size**. `contributing/development.md` carries **six of ten gates** and omits rule 9's cross-target addendum entirely — the rule whose omission left `main` red for three rounds. Transcription from settled records; no code, no new measurement |
| **2** | **Move the RFC 133 probe binary out of the published `prikk` crate** | Product | **RULED 2026-09-09 (§6d.5); required before the next release that publishes `prikk`.** `[[bin]] rusage-object-index-probe` and `[features] rusage-probe` are instrument-only but sit in a published crate, where the feature name becomes durable surface. `tools/corpus` and `tools/benchmarks` are `publish = false` for exactly this; RFC 139 increment 2 already ruled the executor takes the binary path explicitly rather than relying on `CARGO_BIN_EXE_`. Needs a handoff |
