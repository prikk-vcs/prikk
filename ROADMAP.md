...
| "ref is not published" / "received ref does not exist" answer three different classes across commands (`refs.rs:148` `Integrity`, `rollback/draft.rs:216` `InvalidName`, merge's `Integrity`) | DC-75 two-edits inventory, 2026-09-15 | one cross-command sweep in the RFC 132 mould, not a per-command fix |
| **The default key id collides across installations** — `setup` adopts every new maintainer key as `maintainer` and ignores `PRIKK_MAINTAINER_KEY_ID` (measured 2026-09-15). A repository adopts one key per id, so two default installations cannot trust each other, and cross-repository `sync accept` fails on the claim's signature. Affects every multi-party use: planeter's forge, and sync between two people | planeter letters, 2026-09-15 | a design question (per-key default ids, or trust keyed by key rather than id): own RFC, before a forge relies on sync |
