...
| **0.41.0 — "signing readiness"** — **CUT AUTHORIZED 2026-09-12**; prep handed to the dev team (`119-…/release-0.41.0-prep-handoff-v1.md`, first use of the template). Contents: RFC 150 `key status`, RFC 148 window closed, `setup` reuse + synopsis, `rollback/` + `merge/`. **RFC 149 and RFC 130 §8 move to 0.42.0 "the store stops growing"** — stikk is waiting on `key status`, and the crate cut is weeks. | | | |
