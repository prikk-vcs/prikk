...
| **1** | **RFC 144 increment 2 — the same resolution on the seal path** | Product | **RULED 2026-09-09 (§4j.1), and a prerequisite to rename authoring.** `derive_next_state_root` folds `apply_state_effect` one operation at a time, so **a rename cycle cannot be sealed into history today** — increment 1 taught the read path to materialize something the write path still cannot produce. **Constraint: every patch set that derives a root today must still derive the same root** — turning an error into a value is safe, turning a success into an error is not, except for §4j.2's chained rename |
