...
| **1** | **RFC 144 increment 3 piece 3 — the thirteenth conflict witness** | Product | **HANDED OFF 2026-09-09** (§4o.5; `rfcs/handoffs/144-two-point-comparison/increment-3-piece-3-thirteenth-witness-handoff-v1.md`). One node, two disjoint destinations — **nothing fires today**, verified: `classify_path_relation` keys on a `newly_occupied` *intersection*, and disjoint destinations do not intersect. **Enabling change established up front**: `Action::RenamePath` carries only `node_id`, so the node→destination pairing is unrecoverable and the witness cannot be computed without adding it. `SamePathCreate` untouched — §4i.2's dual |
