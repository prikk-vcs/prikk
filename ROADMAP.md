...
| **1** | **RFC 132 — three trust-path preconditions misclassified** | Product | **RULED (RFC 147 §2d) and handed off 2026-09-12** at `rfcs/handoffs/132-error-taxonomy-structure/trust-precondition-sites-handoff-v1.md`, in RFC 132's established per-site mould — **not increment 2, not waiting on RFC 147's Case A/B**. Three sites → `Precondition`. Site 1 is a wording decision, not a variant swap: `Ok(None)` is also what a header-truncated container replays as, so the message leads with *nothing adopted yet; run `trust maintainer add`* without claiming the damage case impossible. Three perturbations required, one per site; `first-run.md` quotes site 1's old message and must move in the same round |
