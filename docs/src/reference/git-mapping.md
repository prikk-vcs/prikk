...
| `git show` | `prikk show <block-id\|patch-id>` | **Changes, not content.** Shows what one block or patch changed, as operation spans rather than a unified diff; it replays nothing, so it prints no file content. |
