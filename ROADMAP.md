...
| **1** | **RFC 131 — lift §6a's prohibition, then narrow §3's visibility** | Product | **AVAILABLE 2026-09-10, not yet ruled (§6c.8).** The gate now sees all three cycle shapes §6a was protecting against, so *no two of the constrained seven may share a group*" has no mechanical reason to stand — and `pub(in crate::<path>)`, inexpressible for those seven while they were forbidden a parent, becomes expressible. **Lifting is on evidence and is RFC 131's own decision**; deliberately excluded from closing the allowlist |
