...
- [`142-content-surface.md`](rfcs/proposed/142-content-surface.md) — RFC 142, Showing what changed (opened 2026-09-08 as item 1. **The finding that shapes it: a patch is self-describing** — `EditText` carries `old_span_text` *and* `replacement_text` verbatim, so showing what a patch changed needs **no replay**, at most one blob read per file. **Ruled: `show` ships, `diff` does not** — an arbitrary two-point comparison needs two replays and inherits the depth^1.45 cost RFC 136 §9.3 measured. Paths for the three node-addressed kinds cost one replay per invocation and are resolved anyway, because a surface showing a human a node id where a filename belongs has failed. **Needs owner acceptance**)
...
measurement corpus, increment 1 shipped in 0.35.0, increments 2-4 open), and **141** (publication
through CI, owner-accepted 2026-09-06 the day it was opened). **140 left `accepted/` for `done/` at
