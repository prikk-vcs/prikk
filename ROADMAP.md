...
| **1** | **RFC 144 increment 3 piece 2 — the commit-time move hint** | Product | **HANDED OFF 2026-09-09** (§4o.4; `rfcs/handoffs/144-two-point-comparison/increment-3-piece-2-commit-time-hint-handoff-v1.md`). **`prikk mv` is undiscoverable to the person who needs it** — a shell `mv` then commit gives delete+create with nothing naming the command. Exact content equality is the whole signal (similarity scoring ruled out); **ambiguous sets emit nothing**, since a hint naming the wrong pair invites the user to assert something false. **Must not be mistakable for the disclosure lines** on the same surface: those are facts about an assertion made, a hint is a suggestion about one that was not |
| **2** | **RFC 144 increment 3 pieces 3-4** — thirteenth witness, structural honesty invariant | Product | **DESIGNED 2026-09-09 (§4o.5-§4o.6), not yet handed off.** The thirteenth is one-node-two-destinations (`SamePathCreate` stays; relabelling deferred). **Piece 4 changes read types across `show`, `bundle preview` and `checkout --patch-plan`** and wants its own round |
