...
| **1** | **RFC 144 increment 3 piece 1 — `prikk mv` and rename authoring** | Product | **DESIGNED and HANDED OFF 2026-09-09** (§4o; `rfcs/handoffs/144-two-point-comparison/increment-3-piece-1-declared-move-capture-handoff-v1.md`). **The first thing in prikk that ever emits a `RenamePath`**, which makes increments 1-2's rename paths load-bearing rather than precautionary. `prikk mv` both moves and declares, with a **record-only row for the shell-`mv`-then-remember flow** that would otherwise make it useless; declarations collapse to the net move per §4j.2, so authoring never emits what replay would reject |
