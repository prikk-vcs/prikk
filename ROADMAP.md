...
| **1** | **`prikk setup` on an existing repository must refuse before touching anything** | Product | **Owner-raised 2026-09-12, measured on the published 0.39.0 binary, handed off** at `rfcs/handoffs/135-first-run-entrance-and-configuration/setup-existing-repository-handoff-v1.md`. Re-run on a directory holding a repository still prints `initialized Prikk repository at …`, mints two keys, then fails — with the corrected `precondition not met` class, because the per-site round fixed the *class* and the docs round *described* the half-run; **nothing fixed the behaviour**. `setup.rs:74-78` has no existence check before `RepositoryLayout::init`, which is idempotent. **The architect's "queue is empty" was false here**: a documented-not-fixed defect does not show as a row |
| **2** | **RFC 131 §6e — grouping census of `prikk-store`'s 52 top-level modules** | Product | **Owner-raised 2026-09-12 ("bloatedness"), measured, handed off** at `rfcs/handoffs/131-module-grouping-and-visibility-scoping/grouping-census-handoff-v1.md`. Today: **102 top-level entries** (123 at opening, 100 after §6b, two added since), 52 modules, 86,137 lines / 276 files / 47% tests, **8 declared coupling cycles — RFC 130's baseline, unchanged**. `pub(in crate::…)` 0 → 61. **The grouping half is a sixth done and had no next increment; the architect had called it "unscheduled by design" — withdrawn.** Census first, per §6d.7's own lesson; the first grouping increment is chosen from it, one family at a time, under §4's must-nots. Cycle removal is a separate theme, scheduled by the owner |
