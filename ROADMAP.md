...
| **1** | **Teach the coupling gate qualified module names** — RFC 130's gate | Product | **OWNER-AUTHORISED and HANDED OFF 2026-09-10** (RFC 131 §6c; `rfcs/handoffs/131-module-grouping-and-visibility-scoping/qualified-module-names-handoff-v1.md`). The gate's node is the top-level `mod` and the scan reads **only the first segment after `crate::`** — the same fact twice, which is why grouping merges nodes and **hides a cycle inside a group**. **Two rulings bound it**: the edge *vocabulary* is unchanged (no `super::` scanning — that changes what an edge means, not how precisely a node is named), and **newly-visible cycles are measured, not remediated** — the round is expected to leave `boundary-check` failing, with the failure enumerated |
