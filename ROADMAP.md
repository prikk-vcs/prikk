...
| **2** | **Move the RFC 133 probe binary out of the published `prikk` crate** | Product | **RULED 2026-09-09 (§6d.5); required before the next release that publishes `prikk`. Handed off** at `rfcs/handoffs/133-performance-cost-and-its-evidence/probe-binary-out-of-published-crate-handoff-v1.md`. `[[bin]] rusage-object-index-probe` and `[features] rusage-probe` are instrument-only but sit in a published crate, where the feature name is durable surface. **Goes to the existing `tools/benchmarks`** — already `publish = false`, already depends on `prikk-store` alone, already in `boundary.rs`'s member allowlist, so **no new workspace member and no allowlist edit**. Test locates the binary by explicit path per RFC 139 increment 2's own ruling |
