...
- [`149-layering-prikk-store.md`](rfcs/proposed/149-layering-prikk-store.md) — RFC 149, layering `prikk-store` (opened 2026-09-12 on the owner's instruction to reconsider the bloatedness direction). **Measured: the six `DECLARED_CYCLES` members form a core that imports none of the 26 surface modules — zero edges, zero through 264 root re-exports** — so a one-directional cut above the core exists on today's code, pending `boundary-check --graph`. The surfaces are where RFC 142–147 landed their growth. Direction: the store keeps below+core and stops growing; the surfaces move to a new crate the owner names. RFC 130 §6 ruled on extracting a module, not on cutting a layer. **Owner rulings required** (§7)
...
| **7 → 0.42.0** | **RFC 149 — layer `prikk-store`: core stays, 26 surfaces move above it** | Product | **Direction APPROVED by the owner 2026-09-12** — *"approved to split … not forced … finally clean, safe and secure, robust and sophisticated design"*, now the RFC's acceptance criterion. **Pending: the surfaces crate's name** (architect recommends `prikk-operations`). Measured cut: zero core→surface edges, zero through 264 root re-exports. Sequenced after step 0 and a public-API census; one move commit per family |
...
| **9 → 0.42.0** | RFC 130 §8 — file-size gate (1,200 lines, reasoned allowlist) + release-evidence size line | Tooling | **Awaits ruling**; interim, useful after the cut |
| **10 (0.42.0 prep)** | Help-versus-inventory check: diff the binary's invocation heads against `commands.md`'s, beside RFC 146 §8e's control | Tooling | **Ruled yes 2026-09-12** at the 0.41.0 prep review — it found `bundle preview` missing for two releases by hand; ~40 lines; its own small round before the 0.42.0 prep |
...
| **RFC 149** — layer `prikk-store` above its cyclic core | `rfcs/proposed/149-…` | ~~direction~~ approved; **name the surfaces crate** (`prikk-operations`?); gate before or alongside |
