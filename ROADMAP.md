...
| **1** | **RFC 144 §4r.1 — make the thirteenth witness reachable from `merge-evidence`** | Product | **RULED 2026-09-09; handed off** at `rfcs/handoffs/144-two-point-comparison/increment-3-piece-3a-flat-sequence-gate-handoff-v1.md`. **Must not become rename-specific** — a special case for `RenameDeferred` that leaves symlink deferral pre-empting classification closes the reported gap and leaves the real one. The risk is not the feature but the **twelve other witness kinds** this shared ordering moves underneath |
