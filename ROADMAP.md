...
| **2** | **Commit memory — re-measure the resident index unmasked** | Product | **RULED 2026-09-09 (§6d.3).** Shrink the probe worker's 11,380 KiB floor and/or extend the ladder past 64,000, so the range rests on more than one point. **Standing control ruled for every future memory probe: resident cost must be >= the structure's serialized size**, or the reading is a measurement failure, not a finding. Handed off at `rfcs/handoffs/133-performance-cost-and-its-evidence/object-index-remeasure-unmasked-handoff-v1.md` |
