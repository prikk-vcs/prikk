...
| **15a — before RFC 136 increment 2; blocks the 0.43.0 cut** | **Merge refuses a side that edits one file twice after the baseline** (found at 1b's review, reproduced on the pre-1b and post-1b builds): two text edits of one file on either side make a confluent merge refuse with `pair_replay_failed`, reported as `integrity error`. Pre-existing, very likely since merge shipped (0.19.0). The cause is to be found by a failing test first; the refusal class is ruled `Precondition` | dev team | `handoffs/DC-75-merge-block-lineage/merge-refuses-a-side-that-edits-a-file-twice-handoff-v1.md` |
