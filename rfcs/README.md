...
| 144 | [Two-point comparison](./proposed/144-two-point-comparison.md) | Unscheduled. **Opened 2026-09-08 on the owner's instruction**, reopening the `prikk diff` refusal of RFC 142 §5/§7. **Reopened because the architect's own stated reason expired**: §5 refused on *cost* — two unbuilt replays — and RFC 143 shipped, exposed and measured endpoint reconstruction. **What survives is semantic**: `commit` never authors a rename (`node_authoring.rs:12`), so a two-point comparison would report the commonest refactor as delete+create. **§4 records that the real question may be whether `commit` should author renames at all**, which is a patch-model change, not a read surface |
