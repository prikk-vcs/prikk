...
| **1** | **RFC 139 increment 4 — the second profile** | Product | **OWNER-APPROVED 2026-09-09.** The cheapest large win: it converts *measured against prikk* into *measured against more than prikk*, removing RFC 136 §9.1 limit 2, which currently bounds every performance conclusion this project draws. **Must prove the contrast with a number before building** — a concentration metric computed for `prikk-self.toml` first, then for the candidate; a profile that resembles prikk has not done the job. Handed off at `rfcs/handoffs/139-measurement-corpus/increment-4-second-profile-handoff-v1.md` |
| **2** | **Commit memory on the node-count axis — measure it, then cut what is provably redundant** | Product | **RULED 2026-09-09 (RFC 133 §6b) at the owner's instruction that it be *recorded and started to be in control now*.** Commit memory is O(nodes ever created): `seen_ids` never shrinks, and `latest_tombstone_by_id` — the larger term — grows with every deletion. **Step 1 is measurement, handed off 2026-09-09** at `rfcs/handoffs/133-performance-cost-and-its-evidence/node-count-memory-measurement-handoff-v1.md` — the count axis past 8,000, multiple samples, and on the **incremental** path (§2's series is genesis, which the owner ruled out of scope in 2026-07-30; the in-scope property has never been measured against node count at all). **Step 2 removes `seen_ids`**, provably redundant against an invariant `cache_ladder.rs:285` already validates. **Neither stops the growth** — §6b.3 step 3 (can tombstones be bounded?) is the one that decides whether the cost is bounded at all |
...
| **3** | **RFC 144 — `bundle preview` on an unsealed repository** | Product | **RULED 2026-09-09 (§4n.1); blocks any release shipping `bundle preview`.** `prikk init` publishes no ref, so the one moment a user most needs to ask what a bundle contains — just initialized, bundle in hand, nothing of their own — returns exit 1. A fifth connectivity state, **not** an overloaded `FastForward`, which would falsely assert their history is a prefix of the bundle's. Handed off at `rfcs/handoffs/144-two-point-comparison/bundle-preview-unsealed-repository-handoff-v1.md` |
