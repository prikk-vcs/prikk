...
| **0.42.0 — RELEASED 2026-09-13** at `d97e467e` (tag `edcdbf76`, signed) | | | Second dev-team-prepared release. CI 16/16 on the release tree before the tag; Release 5/5, 16 assets, not a draft. Shipped Linux asset: checksum OK, build-info `d97e467e`/`0.42.0`, `prikk 0.42.0`, the team's 49-check `smoke.sh` green against it. Contents: RFC 151 (current branch, `branch switch`), RFC 142 §7a / RFC 140 (queued patch message, `show` on a queued patch), RFC 147 §3f, `doctor`'s pointer issue; tooling: size gate, layer rule, lifecycle gate, CI policy job, derived crate order. **crates.io publication not yet authorized.** |
