...
| **1** | **RFC 148 — a default key directory, two `0600` files, env variables as overrides only** | Product | **ACCEPTED 2026-09-12; handoff live** at `148-key-material-across-sessions/default-key-directory-handoff-v1.md` — amended twice that day on the owner's questions: first the environment channel went; then *"requiring paths depends on an individual user"* replaced it with a per-platform default (`$XDG_CONFIG_HOME/prikk`, `%APPDATA%\prikk`). A fresh shell needs **zero** exports. `app-json-settings` evaluated on merit (serde + serde_json): not needed for a directory and two files; the candidate when `prikk config` opens. Breaking-once CLI change: `PRIKK_*_SEED` becomes a one-release loud refusal, `PRIKK_*_SEED_FILE` the only channel. Migration measured: 3 source files through one reader, 36 test files through one helper, 8 docs pages, `ci.yml`, `tools/corpus`, 5 sites in stikk's seam. Ships with the item below — same command, same page |
...
| **3 → FIRST** | **Object-container writes protected against each other — a measured integrity defect** | Product | **RULED at the checkpoint 2026-09-12 (handoff v2): mechanism A, a leaf lock inside `append_object_to_container`, closing all eleven callers at the one funnel.** The team's map was accepted whole. **The architect's own measurement moved it to the front: twelve concurrent `tag create`, four fresh runs, four repositories failing `verify` and `tag` erroring** — the wrong-offset mode their §3 predicted, repository-wide, every time — and **no command reaches `rebuild_index_from_containers`**. Pre-existing since format 3. Overlapping mutators will now refuse visibly (`### Changed`) instead of corrupting silently (`### Fixed`) |
| **3b** | **`prikk doctor --repair-index`** — wire the rebuild that exists | Product | **Handed off** — `102-container-based-durability/doctor-repair-index-handoff-v1.md`. Repositories damaged before the lock lands need it regardless; the function's own doc names the gap |
