...
| **1** | **RFC 144 increment 1 — teach `patch_replay` to apply `RenamePath`** | Product | **ACCEPTED 2026-09-09; handoff issued.** **A gate, not a follow-up** (§4h.7): `lifecycle_cache` applies renames correctly while `patch_replay` — the path behind `checkout` and content-at-a-point — refuses them, so authoring a rename first would seal history `checkout` cannot materialize. **No user-visible change; it unblocks everything else in RFC 144** |
