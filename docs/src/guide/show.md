...
is reconstructed by replay instead). The exception is a checkpoint — a ref's first sealed block, and every 64th block
after it along its history — which stores the content of every file live at that block, edited or
not, so content that was live at a checkpoint reports normally. If a referenced blob is simply **absent** from the object
