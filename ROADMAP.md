...
| ~~**15 — 0.43.0 THEME "sealed snapshots"**~~ | **RFC 136 §10 — SHIPPED in 0.43.0 (2026-09-16), release commit `f3860e28`; RFC 136 moved to `done/`.** Checkpoints at a ref's first block and every 64th; worktree writes anchor at a replay-verified snapshot, read-only reports at the nearest valid one (depth 256: 13.4 s → 4.0 s); provisional worktrees behind a derivation gate; `verify` checks every snapshot. Shipped alongside: stikk 012 and 013, the edited-then-deleted export/sync/rollback fix, the DC-75 two-edits merge, and the Windows mutation suite back to green. **Increment 2c follows 0.43.0**; the cadence question stays open | Product | **DONE** |
