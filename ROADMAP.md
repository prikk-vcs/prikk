...
| **15 — 0.43.0 THEME "sealed snapshots"** | **RFC 136 §10** (design written 2026-09-13): v2 manifest = the state root's leaf set; checkpoint every 64 blocks and at genesis, deterministic; readers anchor at the nearest snapshot; provisional materialization behind a derivation gate; `verify` unchanged | Product | **APPROVED 2026-09-13**; increment 1 handoff live — `136-…/sealed-snapshots-increment-1-handoff-v1.md` **Held by the dev team before code 2026-09-13**: the readers treat a snapshot as the block's *pre*-state, §10.1 defines its *post*-state; ruled §10.1a (post-state; v1 retired; one seal function first); increments re-sliced 0 / 1a / 1b / 2 / 3, handoff v2 live **Increment 0 accepted 2026-09-15 (`9d227afe`)**: one `seal_block`, four callers, block ids byte-identical (verified on the old tree); 1a is next **Increment 1a accepted 2026-09-15 (`cb7d940a`)**: readers on v2, v1 retired, one loader with the recompute check, a directory-sync defect in the replay materializer found and fixed; 1b next, with two rulings (store content Blobs; drop E3) **1b held by the dev team 2026-09-15 before commit, ruled the same day (RFC 136 §10.3a)**: a checkpoint changes cost, never output. 1b anchors no reader; rollback and inverse never anchor; `coverage` keeps its whole-chain meaning; increment 2's anchor is limited to state-only outputs; **no release cut between 1b and 2** **1b accepted 2026-09-15 (`fdbdfd79`, `f3723bd4`)**: checkpoints at genesis and every 64 blocks, edited content stored, no reader anchors, one manifest validator for loader/verify/export; about 108 bytes per entry. Next: the merge defect (row 15a), then increment 2 **Next: increment 2, split 2026-09-15 (RFC 136 §10.3c)**. 2a is the provisional marker, the derivation gate, and anchored read-only reports: `136-block-aggregation-payoff/sealed-snapshots-increment-2-handoff-v1.md`. 2b is a replay-verified block record before any anchored worktree write, since a received block can sign a root its patches do not produce. 2c is baseline reconstruction, where tombstones and `seen_ids` are history. No cut before 2b **2a accepted 2026-09-15** (`14ac8b3f`..`2d96196b`): provisional marker with compare-and-remove clearing, the gate over eight commands, anchored read-only reports (`--patch-plan` 0.28× at depth 256). **Next: 2b**, `136-block-aggregation-payoff/sealed-snapshots-increment-2b-handoff-v1.md` **2b accepted 2026-09-15** (`512a409b`..`1a492d72`): replay-verified record; anchored worktree writes 0.31× at depth 256, byte-identical on the binaries. **Next: the checkout-refusal fix (row 15c)**, carrying the fixture gating and the `#[non_exhaustive]` marks; then increment 3; then 0.43.0 prep. 2c after 0.43.0 unless measured **Checkout-refusal fix closed 2026-09-16. Increment 3 accepted 2026-09-16** (`fef13536`, `cacaeb73`): at depth 256 materialize 13.4 s → 4.0 s and `branch switch` 13.7 s → 4.0 s (exponent 1.33 → 0.48, flat past 128), peak memory 1.19× worst, gate end-to-end asserted, no hard stop; storage 0.2 % per checkpoint at 64 but 30.6 % of the 256-depth repository (44.2 % of the pre-checkpoint one) — an **open cadence question** (§9.3.3), recorded not acted on; **2c confirmed for after 0.43.0** by measurement (`merge-evidence` and cold `commit` untouched at ~1.35). The round found a **product defect held for review** — `bundle export`/`sync build` fail on any history deleting a previously edited text file, broken since 2026-08-10 — ruled and handed off. **Export/sync fix DONE 2026-09-16** (`1df0be33`, `5cc9138a`). **Derivation follow-ups DONE 2026-09-16** (`4885e61b`). **Declarations round DONE 2026-09-16** (`ee86aebe`, `e1f7a412`; RFC 147 §2f delivered — every declaration reports what commit will do with it, and every refusal names routes measured to work). **0.43.0 CUT AUTHORIZED by the owner 2026-09-16; held on red CI and RELEASED FROM HOLD the same day — CI fully green (all 16 jobs) on `353c7464`, the first green run since 2026-09-14.** Was: `ci.yml`'s Windows mutation suite red since 2026-09-14 (`95c74055` was the last green run; twelve failures since, eleven of them architect pushes, all the same job) — two tests, both diagnosed, `DC-87-windows-mutation/windows-mutation-suite-red-handoff-v1.md`. The release commit `dee02867` was never pushed and is dropped; it is redone once the job is green. The sweep itself is accepted (`9253dee4`, `3e5bb3c0`; reviewed `release-0-43-0-prep-review-v1.md`), and the prep template gains a step 0: *CI is green before the sweep starts*. Prep handoff live (`119-release-policy-reset/release-0.43.0-prep-handoff-v1.md`), CHANGELOG date 2026-09-16, previous tag `0.42.0` at `edcdbf76`. **crates.io publication is not authorized** — it needs the owner's own word each release. At the cut the architect moves RFC 136 to `done/`, closes this row, tags and publishes the release, then writes the stikk (letters 012 and 013) and planeter release letters; RFC 136 moves to `done/` at the cut |
...
| **0.43.0 release commit, take two** — sweep accepted (`9253dee4`, `3e5bb3c0`); CI green, so the three-file release commit is redone, still dated 2026-09-16 | `119-…/release-0.43.0-prep-handoff-v1.md` §5 |
