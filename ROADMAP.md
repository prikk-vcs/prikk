...
| item | handoff |
|---|---|
| ~~A deletion of a previously edited text file has no stored preimage Blob, so `bundle export`, `sync build` and sealing its inverse all refuse~~ **FIXED 2026-09-16** (`1df0be33`, `5cc9138a`; reviewed `export-of-an-edited-then-deleted-file-review-v2.md`): replay derives the content, exporters carry it, the seal stores it, receivers unchanged — verified by the architect end to end, including a 0.42.0 binary importing a new bundle. **Five follow-ups handed off** (the refuted merge limit and its control, chain errors swallowed as "nothing derived", `has_object` instead of full reads on the seal path, a false rollback claim in `backup-restore.md`, the apply-layer id-mismatch control) | `DC-78-…/derivation-follow-ups-handoff-v1.md` |
