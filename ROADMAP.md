...
| **7 (0.42.0)** | **RFC 149 — layer `prikk-store`: core stays, 26 surfaces move to `prikk-operations`** | Product | **ACCEPTED 2026-09-12; name `prikk-operations`; gate before the cut.** **Step 1 CONFIRMED 2026-09-13** (partition 5 core + 18 infrastructure / 26 surfaces, zero lower→upper edges, re-derived by the architect; two of the architect's premises corrected in §4a). **Census resolved 2026-09-13 (§5.2b): 41 become `pub`, 10 test-support, 26 already public — every row re-read at file:line.** **RULED 2026-09-13: the 41 approved; notice to stikk required, not gating.** **Increment 1 DONE 2026-09-13 (`4c7019b4`)**: contract in place, 269 → 306 exports; hard stop ruled — five signature types widened (§5.2c). **Increments 1 and 2 DONE (`0184f7bb`, `73a9a378`)**: contract 269 → 318, `prikk-operations` empty and registered in six lists; `CRATE_ORDER` ruled to RFC 141 §7a. **Increment 3 (the moves) starts when CI is green on this push.** 174 of 269 root exports will relocate |
