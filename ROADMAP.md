...
- [`146-machine-readable-listings.md`](rfcs/proposed/146-machine-readable-listings.md) — RFC 146, `--format json` for the three listing commands (opened 2026-09-10 for an idle team). **`prikk log`, `branch` and `tag` have no `--format json`; nine other commands do**, and no principle separates them. **Not a defect against RFC 121** — its §117 put JSON for commands lacking it explicitly *out* of scope, so this is that scope being taken up. **Stands on consistency alone and decides nothing about RFC 145**: useful under every shape and under none. Binds six rules already settled elsewhere — JSON must not fail where prose succeeds (RFC 140 §7b), degrade *absence* only (RFC 142 §6b), machine-branchable fields, empty is a value, nesting is the data, exit codes unchanged. **No serialization dependency**: all six existing emitters hand-build their JSON, and three listings are the wrong reason to spend one of prikk's five runtime crates. **Handoff written and explicitly void unless the owner accepts** — `rfcs/handoffs/146-machine-readable-listings/listing-json-handoff-v1.md`
