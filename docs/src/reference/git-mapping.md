...
| N/A — no counterpart | [`prikk key generate`/`prikk key public`](../guide/first-run.md) | Draws a fresh Ed25519 seed from the OS CSPRNG, or derives a public key from a seed already held. Git has no equivalent — an SSH key or GPG key is generated with a separate tool entirely. |
