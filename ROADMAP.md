...
increment 1 and §7b delivered, increments 2-4 open), **142** (the content surface, owner-accepted
2026-09-08 the day it was opened, handed off), and **154** (adopting a trusted, fast-forward ref advance
without re-signing; owner-accepted 2026-09-16, unimplemented, after the key-id collision fix and RFC
155), **155** (the repository-complete artifact; owner-accepted 2026-09-16, unimplemented; its
all-or-nothing import gets a design round first), and **156** (one object, several signers — a verified signature union
instead of first-copy-wins, a limit of 4 counted signers; owner-accepted 2026-09-16; delivered in 0.45.0), **153**
(`prikk diff`, amended by its §7; owner-accepted 2026-09-17; handed off for 0.46.0) **157** (`prikk tree` and
`prikk cat`, a tree listing and a file's bytes at a point; owner-accepted 2026-09-17; handed off for 0.46.0) **158** (large objects: a bound, streaming, chunk manifests as format 8, and `prikk reclaim`; owner-accepted 2026-09-21, unimplemented, from 0.47.0), and **160** (guards against costs that follow the store and against lengths read from disk — seven measures after RFC 102's whole-container reads; owner-accepted 2026-09-26, scheduled for 0.48.0 after the RFC 102 append-length round), and **161** (release notes a reader and a consumer can use — `### Security`, `### Upgrading` and `### Output changes` blocks, one breaking spelling, a tag message that links its notes, then checks and a per-series archive; owner-accepted 2026-09-26; the section shape and tag message apply from 0.48.0 prep, the checks and archive in a later round). **140 left `accepted/` for `done/` at
...
| ~~**1**~~ | ~~RFC 131 §6d.2 — group one pair, narrow one item, report the cost~~ | Product | **DONE 2026-09-10** (`ec59aba7`). `active`+`worktree_patch` grouped under `commit_boundary`; **the crate's first `pub(in crate::…)` narrowing**, verified by `E0603`; §6d's lift re-proven on real modules; gate green. **A latent gate bug found in passing**: `reexports()` captured only a `pub use` path's first segment, latent since §2.2a and harmless until this round created the first multi-segment one |
| ~~**1**~~ | ~~RFC 131 §6d.3 — re-audit eligibility, comment-aware~~ | Product | **DONE 2026-09-10** (`fc38e853`). All 5 eligible items narrowed, compiler-verified; both ineligible ones actually narrowed, broken at their real callers, and reverted — negative control run, not asserted. **Two honest yields, volunteered against their own headline**: 5/7 declared-ceiling tightening, **3/7 actual access reduction** — `node_authoring` is a private `mod`, so two items were already walled. **My own denominator count counted a doc comment as code** — the same error I had just criticised |
| ~~**1**~~ | ~~RFC 131 §6d.4 — narrow `lifecycle_cache`'s submodule items, no grouping~~ | Product | **DONE 2026-09-10** (`a1ae661a`). **6 of 23 narrowed, both yields identical** — `replay` is `pub(crate) mod`, so nothing was pre-walled and the two numbers are one event; 7 compiler-confirmed ineligible. **10 were blocked by a restriction I wrote**, and the round proved it by narrowing all ten at once and quoting the ten `E0364`/`E0365` errors. It also perturbed the one control nobody would doubt: breaking three `crate::lifecycle_cache::` references made `boundary-check` report a stale `DECLARED_CYCLES` entry, proving the gate is sensitive to coupling and indifferent to visibility. **I under-read my own §6b.2**: its constraint binds items declared *directly* in one of the seven — a **submodule** item already has an ancestor, so `pub(in crate::<module>)` is expressible today with **no grouping**. Measured: submodule items (23 + 19 + 14) outnumber top-level ones (9 + 15 + 10), and cost nothing structural, while grouping one pair cost 7 moves, 21 rewrites, 6 renames and a latent gate bug — for the smaller half |
| ~~**1**~~ | ~~RFC 131 §6d.5 — narrow the re-exports my own handoff put off-limits~~ | Product | **DONE 2026-09-10** (`da8fc40c`). All 10 narrowed at both ends; **`lifecycle_cache` closed at 16 of 23 (70%)** — production 7/14, test-only 9/9, nothing eligible left. Reach reduction proven by me as a **before/after pair** in a detached worktree, not by the after-state alone. My §6d.4 handoff deferred *everything* declared directly in `lifecycle_cache.rs` as *"needs grouping"*. **A `pub(crate) use` re-export is declared there and needs no grouping** — it needs only that its consumers sit inside the module; I conflated a declaration's location with the reason it was blocked. Underneath: `mod cache_ladder;` is private yet its `pub(crate) use` **republishes nine items crate-wide through a separate path**, so the private `mod` walls nothing — **the same declared-vs-actual-reachability class as the `reexports()` gate bug at `ec59aba7`**, once in the gate's model and once in the crate. **All ten audited: zero external consumers, so the ceiling is 16/23** — but **nine are `#[cfg(test)]`**, which neither the report nor my review's first draft said, so the real split is **1 production narrowing (`StoreBackedResolver`, the crate's E1 store boundary) plus 9 test-scope items**. Ruled: a narrowing round must report production and test-only yields separately |
| ~~**1**~~ | ~~RFC 131 §6d.6 — census 37 decorative module gates, narrow `refs` and `text_span`~~ | Product | **CENSUSED 2026-09-10, a negative result** (no code change; tree unchanged at `da8fc40c`). **89 of 91 names are load-bearing** — the narrowing yield in the two assigned files was **0 of 15**. **§6d.4's actual-access test is retired as unsound**: it reads reach off the item's own `mod` line, and here the reach ran through the re-export instead — the origin path was genuinely `E0603`-walled while `crate::lifecycle_cache::StoreBackedResolver` compiled from outside. My test scored the round **zero**; the truth was **ten**. Replacement: reach is what a module outside the scope can *name*, measured by writing the `use` at the parent commit and after. **The shape is crate-wide — 37 sites, 89 republished names, 8 files** including `refs`, one of RFC 130 §2.2's four middle-hubs. **89 is what an origin-side audit misreads, not what is narrowable**; that number is the next round's first task and is deliberately unestimated. Re-export *removal* ruled closed: a re-export carrying a boundary annotation or an established import path is kept and narrowed |
| ~~**1**~~ | ~~RFC 131 §6d.7 — take the last 9 names and close the re-export route~~ | Product | **CLOSED 2026-09-10** (`72f8c60f`). **Route total: 91 censused, 8 narrowable, 8 narrowed, 6 real reductions** — with `lifecycle_cache`'s 10, the route delivered **18 names / 16 reductions**. **My census said 9 narrowable; it is 8** — `RootDirEntry` is pinned by Rust's return-type visibility rule at `read.rs:100`, which **no name-based census can see** because none of the five callers spells the type. Ruled: a name census's narrowable count is an **upper bound**, one-directionally, so §6d.7's 89-load-bearing stands. Two of the eight were bookkeeping not reduction (`mod report;` is private and the two names are not carried onward), volunteered against their own headline for the second time in the arc. **I proved the rule 9 cross-target addendum bites**: breaking `MacosDurability` gives **3 errors on darwin, 0 on the host** — four of six narrowings are invisible to the host gate — now recorded in EXECUTION-ORDER.md §6 rule 9. **The census closes the route: 9 narrowable names of 91.** Both my §6d.6 figures were wrong — 37 was `use` *statements* not sites (25), and my pattern required a statement to end at the name so it **silently skipped every aliased re-export**, making the name count 91 not 89. **And my targeting was wrong for a reason that generalizes**: I aimed the round at RFC 130 §2.2's middle-hubs because §3 says start where the cost is real, and got **0 of 15** — a middle-hub's re-exports are load-bearing *because* it is a middle-hub, so value and availability run opposite. §3 is now qualified in place: **census before choosing the target.** §6d.6's framing is downgraded from an access-surface finding to an **accuracy** one — the `mod` line misleads a reader, but 89 of 91 of those names really are consumed crate-wide. §6d.5's "both ends move together" amended: **only when the outer hop has no independent consumers**, found unprompted by the round on deferred `fsutil` work |
| ~~**1**~~ | ~~RFC 147 — residue: the duplicate `current_target_block`, the dangling symlink, `#[non_exhaustive]`~~ | Product | **Both cases DELIVERED 2026-09-12** (`3fd0ed68`, `d00d7768`, `7c9c06f3`; 1844 → 1857; thirteen gates incl. both cross-target runs). **Case B's site inventory was wrong twice**: §3b said two sites, the round found four, and then claimed a fifth it had not fixed — `patch_inverse/read.rs:16` is a **second** function named `current_target_block`, so `inverse-plan`/`rollback-preview` still mismatch on a tag. **Case A's handoff premise was false** — `From<AuthorError>` carries only the detail into `Precondition`, so status and commit would have disagreed the moment the class changed; the round caught its own first build and fixed it at `node_authoring.rs:227`. **G3, the architect's**: a *dangling* symlink at a tracked path takes the `Missing` branch and the classifier is never asked, while commit refuses — the two fixtures differed by one target file. **RESIDUE DELIVERED 2026-09-12** (`979297e9`, `d1b6be83`; 1857 → 1860; thirteen gates). Fifth site fixed and **a third command found** (`rollback-draft-verify`, hidden behind an empty-WAL refusal); **census done by driving 19 `--ref` invocations**, not by reading the call graph; G3 closed by one expression that removes a syscall and changes only follow/no-follow; `#[non_exhaustive]` on all five, each judged by one rule, **proven from a downstream crate: E0639 on construction, reads compile**. Also ruled there: `#[non_exhaustive]` on `WorktreeChange`, the fourth public struct in three releases to gain a field without it |
| ~~**1**~~ | ~~RFC 147 §3d — `branch create --from` a tag resolves; `tag create --target` a tag refuses; one `current_target_block`~~ | Product | **DONE 2026-09-12** (`df3c57eb`; 1860 → 1863; thirteen gates). **RFC 147 is fully delivered — Case A, Case B, residue, semantics.** The `--from` fix was a **sixth** site, the first outside `prikk-store`; `resolve_ref_tip_block` is now a root export (264 → 265) rather than a seventh copy. The two resolvers consolidated **into `refs`**, not either sibling — both `read.rs` files now mention `refs` zero times, so coupling fell rather than moved. Coverage measured by breaking under `--no-fail-fast`: 184 tests, where fail-fast had reported one. **An architect error found by the round's gate line**: my cross-target counter greps the literal `cfg(target_os` and printed `0` for `refs.rs`, whose three sites are `cfg(all(test, target_os = …))` — the addendum applied and they ran it. `--from <ref>` means "at the block this ref names" — resolve. `--target tags/<t>` is refused as a **precondition naming both accepted forms**, not dereferenced: a tag-of-a-tag is outside the one-hop model and silent collapse would make two requests indistinguishable in history. The two same-named resolvers consolidate to one, direction chosen against the coupling gate |
| ~~**2**~~ | ~~Open the hosting-shape RFC~~ — `prikk instaweb` vs. a separate small server | Product | **OPENED 2026-09-10 as RFC 145** (`rfcs/done/145-serving-a-repository-for-reading.md`), **deciding no build — it rules the shape and hands three questions back**. **Scheduled 2026-09-06, deliberately not first**, per the owner: *"No need to take it as the most prioritized."* Rules which of the owner's two named shapes to build. **Its gate is now open**: RFC 142 shipped the content surface in 0.36.0, so a browse view no longer hits the wall stikk hit — it can show what changed, not only which paths did | **The comparison was missing a shape**: `git instaweb` does not contain a web server, it launches one the machine already has, so **static export served by anything** is what the owner's own example actually is. **The decisive axis is measured, not argued — prikk's entire third-party runtime surface is five crates** (`ed25519-dalek`, `getrandom`, `rustix`, `sha2`, `windows-sys`), and a read-only convenience view is the wrong thing to spend that posture on. **Recommended: static export first; a separate server project held for later; not a serving subcommand** — A's hand-rolled variant is feasible at zero dependency cost, so the objection is ownership, not feasibility: it puts an HTTP parser and a listening socket permanently inside the binary that writes repositories. Also measured: the **library route is already open** (`load_ref_history`, `show`, `ShowPatch` are public), so a separate project needs **no new prikk feature to start**, and the `--format json` gap on `log`/`branch`/`tag` binds only a shell-out implementation. **HELD FOR EXTERNAL REVIEW 2026-09-10** on the owner's instruction — *"I want to discuss and consider very carefully on it, the advancement, before start"* — review request drafted at `external-arch/send/draft/009-…`, and **nothing is to be implemented until that discussion closes**. **§8a records prior art that runs against our own recommendation, before the recommendation**: Fossil ships `fossil ui`, a built-in HTTP server in the same binary that writes the repository, in the project whose low-dependency posture most resembles the one §4 protects; Mercurial ships `hg serve`. **Git is the outlier in borrowing the serving, so the design this RFC calls wrong is what most comparable systems chose** — the recommendation is held at reduced confidence for that reason. **§9a RULED: `instaweb` is not the name, whatever the shape** — the owner observed it *"seems unfamiliar with prikk"* and is right; prikk's vocabulary carries **no portmanteau and no coinage**, and `instaweb` is both plus borrowed. The real name follows the shape. **EXTERNAL REVIEW RECEIVED and ADOPTED 2026-09-10** (reply `010-…`, reviewed at `.git-exclude/reviewed/external-arch-010-hosting-shape-reply-review-v1.md`). **The conclusion holds; its foundation, ordering and shape list all change.** **My decisive axis was a proxy**: the five-crate count is exactly what Fossil beats, and I had already conceded a zero-crate hand-rolled responder — on my own axis I lose my own argument. The real axis is **what capability this adds to the process holding write authority and signing paths**: a listening socket and a permanent inbound attack surface, incurred in full at zero dependencies. **§8a's counterweight is withdrawn** — sorted by *where the browse server lives* rather than by tool, the prior art **supports** the ruling: `cgit`/`stagit`/`hgweb`/`darcsweb`/`darcsden`/Nest all separated, **A tracks monolithic tools and Fossil is the lone pure-A because it had no B available** — a constraint prikk does not have. **A fourth shape, D: ship plumbing, not a server** — prikk's exported read surface *is* the browse substrate, and §6's "B needs no new prikk feature to start" was D already existing. **Reshaped: D the posture, C the first first-party convenience (`export`, honest that content-at-a-point over large history is O(paths x points) and does not export), B the intended dynamic answer rather than an afterthought, A out.** **§8c challenges their own load-bearing assumption and it fails**: `history.rs` imports `RefStore` and `RefStore::publish` is `pub` on that type, so a browse binary linking the read surface links write authority — but it fails *narrowly* (locks are already clean, `prikk-store` already carries a feature, and a feature gate is not the crate split RFC 130 §6 ruled out). Recorded with it: **Rust links a whole crate API, so absence is provable by binary symbol inspection, not by construction, unless a `#[cfg]` gate removes it** — "do not call it" and "do not compile it" are different guarantees. **Four owner rulings required** (§9) **RULED by the architect 2026-09-12 on the owner's instruction (§8d, §9) — acceptance is the owner's.** §8b's shape D read *"commits to a stable-enough read library"*: **that is RFC 132's third re-open trigger verbatim** and contradicts the standing "crate APIs are explicitly unstable" line. **D is re-founded on the CLI** — the surface prikk already commits to (RFC 121, RFC 146 `-v1`) and the one stikk already builds on. A out; **B and C are ecosystem shapes over the CLI, not prikk deliverables**; a first-party `export` is the only convenience prikk would ship and it is unscheduled. §8c dissolves — a subprocess view links no write authority. prikk's obligation is the CLI read surface's completeness, delivered as increments |
| ~~**1**~~ | ~~RFC 146 — `--format json` for `log`, `branch` and `tag`~~ | Product | **DELIVERED 2026-09-10** (`3ccf6f69`), all three schemas verified on the binary, 1824 → 1837 tests, crate count unchanged. **Pushed before review** — the architect read the ahead-list, saw the team's commit, and pushed it under a documentation commit; CI was green on it before it was read, which is luck. Review post-hoc at `.git-exclude/reviewed/rfc146-listing-json-review-v1.md`. **The round found a real dispatch gap** (`prikk tag --format json` never reached the listing; `branch` had the arm, `tag` did not) and fixed it. **Changelog was omitted by the handoff**, closed by the architect. Nine commands answer machine-readably and three do not, with no principle separating them. **Not a defect against RFC 121** — its §117 put JSON for commands lacking it explicitly *out* of scope. **Stands on consistency alone and decides nothing about RFC 145.** Binds six already-settled rules: JSON must not fail where prose succeeds (RFC 140 §7b), degrade *absence* only (RFC 142 §6b), machine-branchable fields, empty is a value, nesting is the data, exit codes unchanged. **No serialization dependency** |
| ~~**1**~~ | ~~RFC 146 §8a — `branch` must filter by ref kind before `-v1` ships~~ | Product | **DONE 2026-09-12** (`fa872c19`), nine lines, verified on a fresh fixture in both forms, 1837 → 1838. **The attached question was measured, not inferred: a received tag *does* appear under `received`.** RULED §8d — **`received` stays kind-agnostic under `-v1` as its contract**: the received structs carry no kind, decoding untrusted received objects to label them belongs to `verify` not a listing, and **a `remotes/tags/` name filter is ruled out** because `validate_received_ref` enforces only the `remotes/` prefix — the remainder is the origin's own name, which this repository does not control. The round also corrected the architect's changelog entry, whose *"fixed in the next increment"* had become false within the same unreleased window — right call, accepted as written. The round flagged, correctly, that `prikk branch` has always listed tag refs — `tag.rs:91` filters on kind, `branch.rs` never did; **verified pre-existing on the shipped 0.38.0 prose**. What changed is the cost: `branch-list-v1` now returns a tag under `branches` with a meaningless `closed` field, **structured data asserting a tag is a branch**. Unreleased today, so a filter fixes it; released, it is a schema change. One question attached — whether `received` has the same hole — to be measured, not fixed silently |
| ~~**2**~~ | ~~`first-run.md` survives neither a reboot nor a second project~~ | Docs | **DONE 2026-09-12** (`6fac3575`), one file, 109 lines, two new H2s; every sequence re-run by the architect on a binary, every cited source line opened, mdBook anchors verified against the generated HTML. **The round found what the handoff missed: the tool already prints the re-export line** (`export …="$(cat ./author.seed)"`) and the page's `--out` example had elided it behind `...`. The second-project flow ran from a `mktemp -d` with explicit `env` — the isolation that makes a broken sequence unable to look correct. Their `--key-id`-must-match test was better built than the architect's, which took three attempts (confounded by a consumed WAL, then by the never-empty-policy guard). **§3a/§3b located exactly** — `trust.rs:214-217` collapses `Ok(None)` into `Integrity` while the damaged case already has its own message at `trust_index.rs:533`; the collision arm returns `InvalidSignature` before any verification — and **a third site found while verifying**: `seal`'s *not trusted by policy* is policy membership, also `InvalidSignature`. **Gap 1:** the page says *"run the printed `export` lines"* and never says `export` dies with the shell — and its own sentence that default `setup` prints both seeds *"once, here, and nowhere else"* **is a data-loss warning written as a security property**: an unsaved seed is permanently lost. **Gap 2:** a second project is uncovered, and the fact that explains it — **trust is per-repository, so an AUTHOR key travels free but a MAINTAINER key must be adopted again in every repo** — is never stated. Verified the reuse flow works in four undocumented steps. **Two product findings reported, not actioned:** `seal` with no key yet adopted says `integrity error: publication trust policy is missing or unreadable` when nothing is missing or damaged (possible RFC 132 re-open trigger), and `setup` on an existing repo prints `initialized` then fails `invalid signature: … already adopted` — wrong axis, though verified to leave the repo undamaged and to leak no seed |
| ~~**1**~~ | ~~RFC 132 — three trust-path preconditions misclassified~~ | Product | **DONE 2026-09-12** (`97f19b8d`). Exactly three variant flips; the adjacent key-binding arm at `trust.rs:335` preserved as `InvalidSignature` and pinned in both test layers; all four rendered lines reproduced by the architect on a rebuilt binary, all exit 1. 1838 → 1844. **The finding behind the tests: a three-site variant change passed the whole suite before this round** — every trust test asserted `.is_err()` only. **Control 6 found four quote sites where the handoff named one**: `beginners_tutorial.rs:97` asserted the exact old line and **would have failed in CI** (the architect's grep had covered `docs/`, not the test tree); `troubleshooting.md`'s H2 *was* the old message, above a body that already described the true cause — the docs were right under the wrong error text for as long as both existed. Three sites → `Precondition`. Site 1 is a wording decision, not a variant swap: `Ok(None)` is also what a header-truncated container replays as, so the message leads with *nothing adopted yet; run `trust maintainer add`* without claiming the damage case impossible. Three perturbations required, one per site; `first-run.md` quotes site 1's old message and must move in the same round |
| **1** | **RFC 146 §8e — the three listing synopses must advertise `[--format json]`** | Product | **FOUND on the published 0.39.0 binary, handed off 2026-09-12** at `rfcs/handoffs/146-machine-readable-listings/help-synopsis-handoff-v1.md`. `commands.rs:173/183/201` lack the flag nine other synopses carry; behaviour verified correct on the crates.io build. **Cause: the architect's own "no prose change" rule, which did not exempt the help synopsis.** The control derives the check from the parser's table so the next command to gain the flag cannot repeat it |
| **0.40.0 — "setup and keys survive a reboot"** | | | **AUTHORIZED by the project owner 2026-09-12.** Every item below is small; together they make the entrance work the day after it is used — and for a team, not a person |
| ~~**1**~~ | ~~RFC 148 — a default key directory, two `0600` files, env variables as overrides only~~ | Product | **DONE 2026-09-12** (`ea0b16f2`, `a0f7b0a2`, `adde6e31`). One round: `setup` on a second project half-ran (init, then refuse on the existing seed); §2c ruled reuse with one precondition phase before `init`. Measured: bare `setup` → sealed commit with zero `PRIKK_*`; second project reuses, seeds byte-identical; one-seed and `0644` states refuse with nothing created. **0.40.0's theme is delivered; item 5 remains** |
| ~~**2**~~ | ~~`prikk setup` refuses on an existing repository before touching anything~~ | Product | **DONE 2026-09-12** (`d871f1e1`). The check sits before `create_dir_all`; controls assert what did *not* happen. **Their correction of the architect's handoff, verified on the published 0.39.0 binary: the old behaviour wrote two orphan `0600` seed files, not "minted for nothing"** |
| ~~**3 → FIRST**~~ | ~~Object-container writes protected against each other~~ | Product | **DONE 2026-09-12 (`e2c86373`), pushed with item 3b.** Leaf lock in the surface-layer wrapper (the `foundation ↔ lock` cycle refused — RFC 102's dated section). **Architect's stress on the fixed binary: 4 of 4 clean** where the old binary damaged 4 of 4; every refusal a lock conflict |
| ~~**3b**~~ | ~~`prikk doctor --repair-index`~~ | Product | **DONE 2026-09-12** (`2ec9aa40` + `00f4d7e7`). Repaired the four repositories the old binary really damaged and a garbage index; returned once because it held no lock (forty repairs beside forty writers broke `verify`), now holds it across scan and install, measured clean. The bypass guard is a table of guarded writers, not one name |
...
| ~~**5**~~ | ~~`checkout --snapshot-plan`'s by-design missing snapshot → `Precondition`; `security-setup.md:22`~~ | Product / Docs | **DONE 2026-09-12** (`f06125f1`, reviewed from the handoff and diff — report still owed). Adjacent missing-Blob arm stays `Integrity`; the AUTHOR sentence now states what is and is not enforced, measured twice. **All five 0.40.0 items delivered; CI green on Windows at `d007d2c7`; the cut is proposed and awaits the owner's word** |
| **0.40.0 — RELEASED 2026-09-12** at `e7279edc` (tag `31481c66`, signed) | | | CI 15/15 before the tag; Release 5/5, 16 assets, not a draft. Shipped Linux asset verified: checksum, build-info commit/tag, `prikk 0.40.0`, and every 0.40 feature on a fresh fixture — bare `setup` to a sealed verified commit with zero `PRIKK_*`, second project reuses, `doctor --repair-index`, retired variable refused, twelve concurrent `tag create` left `verify` clean. **PUBLISHED to crates.io 2026-09-12**: all eight crates at 0.40.0 on the sparse index, none yanked; `cargo install prikk --version 0.40.0` builds and a bare `setup` reaches a sealed, verified commit with no environment. |
| **0.42.0 release position (2026-09-13, evening)** | | | **Theme delivered**: RFC 151 in full, plus the lifecycle gate, the CI policy job with required steps, `size-check`, the layer rule, the derived crate order. **Both stikk rounds landed (`bf36b2ee`, `ee096fd3`, `804b3b51`).** **RELEASED 2026-09-13** — see the release table. |
| **0.42.0 — RELEASED 2026-09-13** at `d97e467e` (tag `edcdbf76`, signed) | | | Second dev-team-prepared release. CI 16/16 on the release tree before the tag; Release 5/5, 16 assets, not a draft. Shipped Linux asset: checksum OK, build-info `d97e467e`/`0.42.0`, `prikk 0.42.0`, the team's 49-check `smoke.sh` green against it. Contents: RFC 151 (current branch, `branch switch`), RFC 142 §7a / RFC 140 (queued patch message, `show` on a queued patch), RFC 147 §3f, `doctor`'s pointer issue; tooling: size gate, layer rule, lifecycle gate, CI policy job, derived crate order. **PUBLISHED to crates.io 2026-09-13**: all eight crates at 0.42.0 on the sparse index, none yanked; `cargo install prikk --version 0.42.0` builds and passes the team's 49-check smoke. |
| **0.41.0 — RELEASED 2026-09-12** at `32bf0006` (tag `525db19e`, signed) | | | **First release prepared by the dev team** (`119-…/release-0.41.0-prep-handoff-v1.md`): their sweep fixed four false `--help` synopses and two inventory gaps before the cut. CI 15/15 on the release tree before the tag; Release 5/5, 16 assets, not a draft. Shipped Linux asset: checksum OK, build-info `32bf0006`/`0.41.0`, `prikk 0.41.0`, the team's `smoke.sh` all 25 checks green against it. **PUBLISHED to crates.io 2026-09-12**: all eight crates at 0.41.0 on the sparse index, none yanked; `cargo install prikk --version 0.41.0` builds and passes the same smoke script. stikk letter 007 sent the same day. |
| **0.41.0 — "signing readiness"** — **CUT AUTHORIZED 2026-09-12**; prep handed to the dev team (`119-…/release-0.41.0-prep-handoff-v1.md`, first use of the template). Contents: RFC 150 `key status`, RFC 148 window closed, `setup` reuse + synopsis, five `--help` corrections from the sweep. (The two groupings shipped in **0.40.0** — the handoff had named them here in error; the team caught it and wrote the release commit's message without them.) **RFC 149 and RFC 130 §8 move to 0.42.0 "the store stops growing"** — stikk is waiting on `key status`, and the crate cut is weeks. | | | |
| **0 — was URGENT** | **`main` was red on Windows from `0609ee51` to `2f9434ac`**: the test isolation seam does not redirect `APPDATA`, so every Windows `setup` test wrote the runner's real key directory in parallel (CI 34686656667, three tests). RFC 148 handoff v3. **Fixed (`2f9434ac`); CI on `d007d2c7` green on every job including the Windows mutation suite.** | Product / CI | Closed |
| ~~**0**~~ | ~~Remove the retired `PRIKK_*_SEED` detection~~ | Product | **DONE 2026-09-12** (`4c30c854`). Measured: a stale variable holding a different seed changes nothing; `key status` names the directory's key; the field never published. **RFC 148 closed.** Windows-side unread assertion: first line of the 0.41.0 prep |
| ~~0b~~ | ~~`setup`'s help synopsis~~ | Product | **DONE 2026-09-12** (`2212f7f9`). Ruled: no description gate; the cut-time reading, now step 1 of the release-prep sweep |
| ~~**0c**~~ | ~~RFC 150 `prikk key status`~~ | Product | **DONE 2026-09-12** (`2212f7f9`). Every state measured against the signing path; the shared-query control is an if-and-only-if. Unblocks item 0 |
| ~~**6**~~ | ~~`boundary-check --graph`~~ | Tooling | **DONE 2026-09-12** (`60f17dcf`): `release-policy-module-graph-v1`, checked against the gate's own hub list |
| **7 (0.42.0)** | **RFC 149 — layer `prikk-store`: core stays, 26 surfaces move to `prikk-operations`** | Product | **ACCEPTED 2026-09-12; name `prikk-operations`; gate before the cut.** **Step 1 CONFIRMED 2026-09-13** (partition 5 core + 18 infrastructure / 26 surfaces, zero lower→upper edges, re-derived by the architect; two of the architect's premises corrected in §4a). **Census resolved 2026-09-13 (§5.2b): 41 become `pub`, 10 test-support, 26 already public — every row re-read at file:line.** **RULED 2026-09-13: the 41 approved; notice to stikk required, not gating.** **Increment 1 DONE 2026-09-13 (`4c7019b4`)**: contract in place, 269 → 306 exports; hard stop ruled — five signature types widened (§5.2c). **Increments 1 and 2 DONE (`0184f7bb`, `73a9a378`)**: contract 269 → 318, `prikk-operations` empty and registered in six lists; `CRATE_ORDER` ruled to RFC 141 §7a. **Increment 3 stopped at the first move, correctly (2026-09-13): five modules stay in the store by nature (26 → 21 families); the tests need a named test-support surface first.** **3a DONE (`d3340a3f`)**: 24 fixture names under the feature, graph +4 fixture nodes, no cycle, no hub. **3a.2 DONE (`bb43c86d`)**. **3b stopped at the first trial move, correctly (fourth time the same shape): the contract was core-only, the move needs the infrastructure too.** **RULED 2026-09-13 — path B.** The crate cut is not made. **RFC 149 DONE 2026-09-13** (`7214c039`, `feec231b`, `2c82fb76`) — moved to `rfcs/done/`. The layer is a gate rule (20/29, 0 violations), one crate, the store's exports are 0.41.0's again by diff against the tag — `149-…/path-b-layer-rule-handoff-v1.md`. **0.42.0's theme is renamed: "the store is bounded and layered by gate."** stikk confirmed it links no prikk crate — §4b corrected; no library consumer known |
| ~~**8**~~ | ~~RFC 131 §6e — grouping census~~ | Product | **DONE 2026-09-12**: census reviewed, §6f ruled and closed — `rollback/` and `merge/` in (52 → 49), `worktree/` refused by measurement; feeds RFC 149's family boundaries |
...
| ~~**15 — 0.43.0 THEME "sealed snapshots"**~~ | **RFC 136 §10 — SHIPPED in 0.43.0 (2026-09-16), release commit `f3860e28`, signed tag `0.43.0`; CI 16/16 before the tag, Release workflow 5/5 with 16 assets, verified on the shipped Linux asset (checksum, build-info, `--version`, 39/39 smoke), and PUBLISHED to crates.io on the owner's word (all eight crates; `cargo install` passes the same smoke); RFC 136 moved to `done/`.** Checkpoints at a ref's first block and every 64th; worktree writes anchor at a replay-verified snapshot, read-only reports at the nearest valid one (depth 256: 13.4 s → 4.0 s); provisional worktrees behind a derivation gate; `verify` checks every snapshot. Shipped alongside: stikk 012 and 013, the edited-then-deleted export/sync/rollback fix, the DC-75 two-edits merge, and the Windows mutation suite back to green. **Increment 2c follows 0.43.0**; the cadence question stays open | Product | **DONE** |
| ~~**15c**~~ — **CLOSED 2026-09-16** | **A refused checkout writes files first and blocks `commit`** (stikk letter 012, measured by the architect on `26e8c528`). `checkout --patch-materialize` writes manifest entries one by one and refuses at the first conflict (`integrity error`), leaving earlier files and the RFC 102 dirty marker set. `status` and `doctor` stay silent, and `commit` then refuses with a route that loops. Recovery measured: materialize or switch to the current branch. Fix: plan every entry before any marker or write (all three materializers); refusal classes per site; `status` and `doctor` report the dirty marker with a true route. **Closed 2026-09-16** (`f91495fe`..`47fb6def`): one planner refuses every write conflict before either marker, for all three materializers; the refusals are preconditions naming every path and a true route; `status` (prose and JSON) and `doctor` report a stopped checkout. stikk's scenario checked by the architect on the binary | dev team | `handoffs/102-container-based-durability/checkout-refusal-writes-nothing-handoff-v1.md` |
| ~~**15a**~~ — **CLOSED 2026-09-15** | **Merge refuses a side that edits one file twice after the baseline** (found at 1b's review, reproduced on the pre-1b and post-1b builds): two text edits of one file on either side make a confluent merge refuse with `pair_replay_failed`, reported as `integrity error`. Pre-existing, very likely since merge shipped (0.19.0). The cause is to be found by a failing test first; the refusal class is ruled `Precondition` **Cause found and ruled 2026-09-15** (handoff §6): the confluence engine judges every operation against the baseline, while authoring sequences edits. Fix is Option A, per-side net-effect folding in evidence only, with a replay-equality guard and report indices kept on the original sequence. Refusal class moved to `Precondition` in `a76ff611`; the reproduction test is `749365ee`. **R1 implemented 2026-09-15 (`b5b92373`..`70afbc8a`, local, not pushed)**: 2 / 0 and 0 / 2 merge, the inventory classes are in, and `merge.md` lists what cannot merge yet. Review found a false `proven_confluent` from the architect's own no-op ruling (a net no-op on the adopted side hides the other side's edit to that file; the merge then fails as damage, with the repository clean). **Fix required before push** (handoff §7.2). Then R6 mixed-kind folds, R7 create-then-delete stays refused, R8 same-node label, R4 create-then-edit (§7.3). **Closed 2026-09-15** (`5ba1a0d3`, `b42744fe`..`7a7072c3`): a no-op run is set aside only when the other side leaves that file alone; mixed kinds and create-then-edit fold; create-then-delete never folds; a same-file predecessor labels `sequence_internal_dependency_deferred`. The architect's both-direction binary run refuses every adversarial case and merges every admissible fold to the right bytes. Still refusing by design: renames (15b), create-then-delete, delete-then-re-create at a path, a restored word beside the other side's edit. Candidate fold: create then mode change | dev team | `handoffs/DC-75-merge-block-lineage/merge-refuses-a-side-that-edits-a-file-twice-handoff-v1.md` |
| **15b — SCHEDULED 0.45.0** | **Implementation ACCEPTED 2026-09-17** (`63917d77`; docs follow-up owed: the one-commit swap merges only against an untouched side). **Design ACCEPTED and implementation HANDED OFF 2026-09-17** (addendum 1: four-piece algebra change, patch-batched rename replay, one round with CLI controls; `144-two-point-comparison/merge-with-renames-design-handoff-v1.md`; measured: even a rename against an untouched side refuses; the replay oracle refuses `RenamePath`, `replay_oracle.rs:124`). **Merge refuses any branch containing a rename** (measured on the binary 2026-09-15: `prikk mv` alone, rename then edit, edit then rename — all `unsupported_operation`). RFC 144 §4r.1's designed deferral (`RenameDeferred`, the confluence oracle has no rename replay), undocumented until the fix round's `merge.md` sentence. Candidate follow-up: rename replay in the confluence oracle. Scheduling is the owner's | owner's scheduling | `rfcs/accepted/144-two-point-comparison.md` §4r.1 |
| **16b — `prikk diff` SCHEDULED 0.46.0; `stash` unscheduled** | **RFC 153 — `prikk diff`** (two points, or worktree vs baseline; declared renames only; line-oriented rendering over reconstructed contents; also closes RFC 144 §4t's bare block-id `--ref`). Owner approved the direction 2026-09-13; scheduling is the owner's. **`stash`** is the approved later candidate after RFC 136 (complement of `branch switch`'s dirty-worktree refusal; needs its own design — the WAL is not a stash). **`git-mapping.md` currency** handed off 2026-09-13 (`128-outward-facing-project-surface/git-mapping-currency-handoff-v1.md`): five stale claims, fetch/pull/push rows, rule (C) test; release-prep template gains step 8 (absence claims) | owner's scheduling | `rfcs/proposed/153-content-diff.md` |
| **17 — unscheduled (after RFC 154/155)** | **RFC 113 — history import foundations**: the import contract on prikk's side, ruled 2026-09-13 (importer signs the import; only an adopted maintainer seals; refuse-not-approximate Git floor); then Git, then Subversion, then CVS; brygge decodes. **Found 2026-09-23 from brygge's letters 001–002: the prerequisite for a large import is `seal`'s cost, and it is unscheduled.** RFC 133 §5b measured per-seal cost linear in depth (cumulative quadratic, 2.03; 23–101 h projected at depth 2,048) because `seal` walks the ancestor lineage to derive the next state root; §5b.2 recorded reducing it as a prerequisite, not scheduled. RFC 136 increment 2c (commit/merge-evidence reconstruction) is a **second** cost an import pays, not this one. **Schedule seal's per-block cost with this theme or before it.** The policy lever is fewer, larger blocks per import (brygge's Q-8). Also owed when the foundations open: `AttestationPayload.created_at`'s doc comment says "authoritative creation timestamp" — ruled the zero no-clock sentinel for an import attestation (brygge Q-3), and the comment is to be corrected in the first round that touches `prikk-object` | Product | **Awaits the owner's scheduling** |
| **18 — key-id collision SCHEDULED 0.45.0; RFC 155 then 154 from 0.48.0** | **Several maintainers, one branch; a whole repository in one file.** **RFC 154 ACCEPTED 2026-09-16** (adopt a received RefState signed by a locally adopted key: fast-forward only, never re-signed, replay-verified, CAS-published; DC-78 §D4 amended in premise, letter kept). Proposed order: (1) the **maintainer key-id collision fix** (§C row), (2) **RFC 155 — the repository-complete artifact** (ACCEPTED 2026-09-16; the all-or-nothing import gets a design round first: every object and ref verbatim; trust policy as inert material, never adopted by import; offline signature check against carried material; all-or-nothing import even when killed; `import --adopt` lands trusted refs via RFC 154; also RFC 114 §5.2's carry-forward), (3) the adoption act. Consumer: planeter (letters 2026-09-15/16) | Product | RFC 154 direction ruled; RFC 155 direction and all scheduling are the owner's |
| **SCHEDULE after 0.43.0 — the architect's, 2026-09-16** | **Defects in how people work together first, then the features consumers asked for, then cost at depth, then the one-file repository.** **0.44.0 — RELEASED 2026-09-16** (signed tag on `20711642`; CI 16/16 before the tag; Release 5/5, 16 assets; shipped Linux asset and `cargo install` each 51/51 smoke; all eight crates on crates.io; security advisory **GHSA-px5q-233r-6hq5** published — a refused `bundle import` left the bundle's objects behind in 0.23.0–0.43.0). A minor, not 0.43.1: *"in v0 a release with any runtime change is a minor bump"* (release-compatibility.md). It carried — a fix already handed to the dev team, **RFC 147 §2h** (`worktree-status` hung on a FIFO at a declared destination in 0.43.0; a non-regular destination is now never read, and reports `null` differences), and **RFC 147 §2g**: a directory at a declaration's destination made `worktree-status` say `rename` while `commit` recorded a deletion and called it ignored (stikk letter 014; handoff `147-…/declaration-presence-is-one-definition-handoff-v1.md`). **0.45.0 "working together" — RELEASED 2026-09-17** (signed tag on `27fcbd76`; CI 16/16 before the tag; Release 5/5, 16 assets; the shipped Linux asset and `cargo install prikk --version 0.45.0` each pass the 104-check smoke; all eight crates on crates.io; advisory GHSA-px5q-233r-6hq5 amended to say the related limitation is addressed in 0.45.0 for format-7 repositories) — (0) **RFC 156 — one object, several signers — ACCEPTED 2026-09-16 and handed off** (`156-…/one-object-several-signers-handoff-v1.md`): a verified signature union instead of first-copy-wins, closing the limitation GHSA-px5q-233r-6hq5 names; Stage 0 measured that a second Block record breaks 0.44.0's `verify`, and **the owner chose a superset repository format 7** reached by an explicit `prikk format upgrade` (RFC 156 §5b; RFC 114 §5.2a records that an in-place upgrade to a superset format satisfies the carry-forward rule) — so 0.45.0 introduces format 7. **Stages 0–2b, 4 and 5 accepted 2026-09-17** (`121e68c6`, `12269445`, `7e9a1f85`, `1a59aecf`; review `one-object-several-signers-review-v1`). **Stage 3 — the bound — accepted 2026-09-17** (`a744759c`; RFC 156 §7.4: provenance is not stored state, so every signature outside verified adopted maintainers counts, new objects included, limit 4, read under the active lock; local writers are never refused); follow-up accepted 2026-09-17 (`fab4aded`): counting, and verifying, stops at the first signature past the limit. **§7.4 confirmed by the owner 2026-09-17** ("Confirmed. Accepted."). **All four 0.45.0 items are complete; the owner authorized the 0.45.0 cut 2026-09-17 ("Yes. Authorized."), and the prep handoff is live** (`119-…/release-0.45.0-prep-handoff-v1.md`). **Items (1)–(3) and the measurement handed off 2026-09-17, in this order of work:** the warm-cache measurement (`136-…/warm-cache-commit-anomaly-handoff-v1.md`, measure-only), the key ids (`135-…/distinct-default-key-ids-handoff-v1.md`), the merge-with-renames design round (`144-…/merge-with-renames-design-handoff-v1.md`, report only), the refusal sweep (`132-…/absent-and-received-ref-refusals-handoff-v1.md`); (1) the **maintainer key-id collision**, measured 2026-09-17 to cover authors too: two default installations cannot exchange any history (every `setup` key is named `maintainer`, so two default installations cannot trust each other; prerequisite for sync between people, planeter and RFC 154); (2) **merge of a branch containing a rename** (row 15b: `prikk mv` shipped in 0.38.0, stikk now surfaces renames, and any renamed file makes `merge` refuse); (3) the **refusal-class sweep** — **DONE 2026-09-17** (`bbdbcb71`, `63f4fc41`, CI fix `6eb102b7`; consumers only, received refs refused only where the answer was false, "no prikk repository at <path>") ("ref is not published" answers three classes; `checkout --snapshot-plan`'s `Integrity`; and `checkout --snapshot-plan`/`--snapshot-materialize --ref remotes/…` answering *"not a checkpoint, so it carries no snapshot"* for a received checkpoint block, measured 2026-09-16 — the true reason is that a received ref cannot be checked out; and `verify` or `format upgrade` on a path holding no repository answering `i/o error: No such file or directory`, measured 2026-09-17); plus a **measure-only investigation of the warm-cache `commit` anomaly** (RFC 136 increment 3) — **DONE 2026-09-17** (`664895b3`, review `warm-cache-commit-anomaly-review-v1`): a legitimate miss by design, not a defect. The DC-64 one-block step cannot materialize text whose content came from an earlier edit, so any tip that edits such a file falls back to full replay (56–71 % of tips on the corpus); 2c stays a performance round, after 0.46.0. **0.46.0 "comparing" — OPENED 2026-09-17 by the owner ("Open 0.46.0")**: RFC 153 amended for 0.43–0.45 (§7: one resolver extending `require_existing_ref`, received refs read, no `untracked` section, fresh-repository rule) and **RFC 157 proposed** (`prikk tree`, `tree-listing-v1`; `prikk cat`, bounded bytes, `path-content-v1`), **both ACCEPTED by the owner 2026-09-17** ("Yes. Both accepted. Write them.") and handed off in this order: (1) the shared point resolver, a ref or a bare block id, wired into `checkout`'s read-only modes, closing RFC 144 §4t (`153-content-diff/point-resolver-handoff-v1.md`); (2) `prikk tree` then `prikk cat` (`157-reading-a-point/tree-and-cat-handoff-v1.md`); (3) `prikk diff`, two points then the worktree (`153-content-diff/diff-handoff-v1.md`). **All three delivered:** the resolver (`13f9ace0`), `tree` and `cat` (`9f7b7108`, RFC 157 complete), `diff` between two points (`5457a931`, accepted with RFC 153 §6a's counted work bound) and **`diff` against the worktree, accepted 2026-09-22** (`5609efc6`; review `diff-worktree-review-v1`; RFC 153 §6b rules the five open questions). The worktree side is read through `commit`'s own parts and writes nothing — the round found that the shared baseline derivation refreshes the rebuildable cache, so `diff` takes the same derivation with the refresh off and `worktree-status`'s "no writes are performed" is corrected. **One fix is owed before the cut** (handoff Addendum 2): a bare diff's left side can be the tip plus its unsealed queue, a state no block names, and the binary `prikk cat` hint prints the sealed tip's block — the hint must say so instead. **F1 fixed and accepted 2026-09-22** (`2e0a3961`; review `diff-hint-and-bound-note-review-v1`). **`main` is red on macOS** at `f2e0e200` — `rfc153_diff_worktree.rs`'s W13 writes a non-UTF-8 filename, which APFS refuses (`Illegal byte sequence`), a case `rfc147_authoring_refusal_field.rs:113-115` had already diagnosed and gated; **`cfg(unix)` states an API surface, not a filesystem capability**. **Fixed and accepted 2026-09-22** (`787cab49`, Addendum 3): the fixture **probes** the capability and skips with a printed reason, and the sweep of all five new test files found nothing else of the class. Pushed together: `5609efc6`, `2e0a3961`, `e3e37393`, `787cab49`. **RFC 153 is closed.** **CI 16/16 green on `44e4ea70`** (macOS mutation suite read by name), and the three stale "no diff" claims in this file dated at `b1b2d762`. **0.46.0 "comparing" — RELEASED 2026-09-22** (owner authorized the cut, *"Approved."*; signed tag `0.46.0` on `d6d21df0`; CI 16/16 before the tag, every job read by name; Release workflow 5/5, 16 assets; the shipped Linux asset verified — checksum OK, build-info naming `d6d21df0` and tag `0.46.0`, `prikk 0.46.0`, and the team's 142-check smoke script passing against the **downloaded** binary, not a local build). The prep round (`release-0.46.0-prep-handoff-v1.md`) carried the owed W13 assertion as item 0; the public API is purely additive (0 removed, 0 changed, 81 added, re-derived by the architect), and the memory ratio moved 6.33x -> 6.24x, inside the method's own swing. **Still outstanding: crates.io publication, which needs the owner's own word, and the deferred stikk/planeter letters covering 0.44.0-0.46.0.** **Consumer letters for 0.44.0–0.46.0 (stikk, planeter) are due at the 0.46.0 release, by the owner's ruling of 2026-09-17** — both consumers stay idle until then; the two unsent 2026-09-16 drafts are folded in then. — RFC 153 `prikk diff`, which also closes bare `--ref <block-id>`, stikk's remaining named dependency; **plus planeter's A1 (a JSON tree listing at a ref or block id) and A2 (file bytes by path at a point, text and binary, with an all-or-nothing size bound)**, which share `diff`'s point resolver — planeter sent the JSON shapes it would consume (2026-09-16), and the design is an RFC to write. **0.47.0 "depth" — first round is a regression fix, ruled 2026-09-22 from stikk's letter 015 (RFC 147 §2i, handoff `147-refusal-visibility/unpublished-current-branch-handoff-v1.md`): naming the repository's **current branch** explicitly, before its first `seal`, is refused as absent by `worktree-status`, `log`, `tree`, `cat`, `diff --from` and `checkout`'s read-only modes, while every bare form answers for that same ref — the sweep conflated "has a published RefState" with "exists", and RFC 157 §3 and RFC 153 §7.3 inherited the word. The architect's defect, shipped in 0.45.0 and widened in 0.46.0. Then the **measurement round — CLOSED 2026-09-23** (controls `9bcdc956`; follow-up `ef120c58`, `f10d2a89`, review `rfc133-measurement-cost-follow-up-review-v1`: per-point elapsed time in both reports; the release-gate profile is ~55 % of the full sweep **within one session**, while the identical sweep took 4,504 s one day and 9,633 s the next — absolute minutes on this shared machine move ~2× with load, so the template records none; the two owed controls delivered). Earlier: **code ACCEPTED 2026-09-23** (`cad7abfc`, `423a8a62`; review `rfc133-measurement-cost-review-v1`), **its wall-clock figure REFUSED**: the release-gate profile is a strict subset of the full sweep and measured *longer* (5,260 s / 5,616 s against the sweep's 4,504 s), which cannot be true — the RSS agrees at every point, so the timing measured the machine, not the profile, and the architect's own gate runs were part of the load. Per-point elapsed timing replaces a third blind 90-minute attempt, and the template carries no cost claim until a clean run produces one (`133-performance-cost-and-its-evidence/measurement-cost-handoff-v1.md`: a second `#[ignore]`d entry point fixed in source, not env knobs, because a stale export silently shrinks a release gate; the gate plan is split out and waits in `.git-exclude/scratch/` until it is next), then **RFC 158 Stage A** (**handed off 2026-09-23**, `158-large-objects/incoming-bound-handoff-v1.md`: refuse before reading on all six entry points, then the per-object bound and `prikk config`) — **ACCEPTED 2026-09-24** (`82359ff4`, `033c6fcc`, `312cb031`; review `rfc158-incoming-bound-review-v1`): all six entry points refuse before reading; peak on an oversized file stays under 64 MiB where 0.46.0 read it whole; a 255 MiB blob still costs `bundle import` ~6.0× and `verify` ~3.0× (Stage B's starting line); **Addendum 1 ACCEPTED 2026-09-24** (`fcca52c6`: a saturating bound, the per-object boundary pinned on both sides, and a file of N bytes documented as needing a bound of N + 69); **Stage A is closed**; then **RFC 136 increment 2c — design round HANDED OFF 2026-09-24** (`136-block-aggregation-payoff/baseline-reconstruction-increment-2c-handoff-v1.md`: measure direction (a) for the DC-65 half; lay out options under §10.3c ruling 3 for the cold half and `merge-evidence`, since a snapshot carries no `latest_tombstone_by_id`/`seen_ids`; nothing product lands until the architect rules) — **design ACCEPTED 2026-09-24** (`4c739cab`, review `rfc136-2c-design-round-review-v1`): (a) plus the anchor-text helper for the `materialize_edited_text` family is 2c's implementation (Addendum 1, live); (i), the split walk, is deferred to be designed with `seal`'s lineage walk (theme 17, owner's scheduling); (ii) rejected. **The round found every corpus timing to date was a debug build** (`locate_prikk_binary` lacked `--release`): cold replay at depth 256 is 14.0 s debug and 0.47 s release; `current-state.md` corrected, and a release re-measurement round follows 2c. **2c implementation ACCEPTED 2026-09-24** (`4ec84b1a`, review `rfc136-2c-implementation-review-v1`; release, depth 256: warm `commit` 513 → 68 ms, `merge-evidence` 4.02 → 0.49 s). **Addendum 2 owed before the cut**: a request for the cached baseline itself fell through to full replay, so every `commit` after the first between seals replayed in full (DC-64 §3.2 never considered it) — **Addendum 2 ACCEPTED 2026-09-24** (`fc2f9753`): a same-baseline request is a hit counted as a step, so later commits between seals drop from ~0.5 s to ~45 ms at depth 256 (release). **Increment 2c is CLOSED.** Then the **release re-measurement — HANDED OFF 2026-09-24** (`139-measurement-corpus/release-remeasurement-handoff-v1.md`: the build curve toward depth 2,048 under a 2-hour-per-checkpoint rule, the two-measurements ladder, the scale page rewritten, and a measured `seal` figure for brygge) — **ACCEPTED 2026-09-25** (`3a627af2`, review `release-remeasurement-review-v1`): in release, `seal` costs 0.64 s at depth 256 and 5.0 s at 1,024, and building to 1,024 takes 34 min; the build is **still roughly quadratic from about depth 128** (cumulative exponent 2.0 over 128–1,024), so the debug build changed the scale (8–33×), not the deep shape; the scale page is rewritten from release figures. **0.47.0's planned content is complete; the owner authorized the cut 2026-09-25** (*"Yes."*), **0.47.0 "depth" — RELEASED 2026-09-25** (signed tag `0.47.0` on `21895f46`; CI 16/16 before the tag; Release 5/5, 16 assets, Latest; the downloaded Linux asset passes checksum, build-info and the 231-check smoke; prep review `release-0-47-0-prep-review-v1`; public API: two option structs and `diff_worktree_reporting_anchor` changed, named in the CHANGELOG; memory ratio 1.910× against 1.930×). **crates.io publication awaits the owner's word.** **After the cut, in order:** brygge letter 004 (the owner sends it after the cut); **RFC 159 — sealing without re-walking the lineage — PROPOSED 2026-09-25 and scheduled by the owner** (*"Schedule it and start the design when ready."*): a design round first after the cut (**handed off 2026-09-25**, `136-block-aggregation-payoff/seal-anchor-design-round-handoff-v1.md`, under RFC 136 because a proposed RFC carries no handoffs; **DELIVERED and reviewed 2026-09-26**, `455eeca7`, review `rfc159-design-round-review-v1`: prototype identity 0 differences over six 1,024-block histories; one `seal` at 1,024 goes 4.1 s → 0.39 s and **1.8 GiB → 30 MiB peak** (the shipped figure reproduced by the architect on the 0.47.0 asset); a 64-block `sync` catch-up 251 s → 18 s; rulings in RFC 159 §8 (C2: an anchor must also carry an adopted maintainer's signature, applied to 2b's worktree writes too); **RFC 159 ACCEPTED by the owner 2026-09-26** (*"Yes. Accepted."*); implementation handed off (`159-sealing-without-rewalking/implementation-handoff-v1.md`) and **0.48.0 is re-scoped to ship it first**, cut as soon as it is accepted, ahead of RFC 158 Stages B and C; **implementation ACCEPTED 2026-09-26** (`0a5d6ead`, review `rfc159-implementation-review-v1`: one `seal` at depth 1,024 1.8 GiB → ~30 MiB, 4.3–4.5 s → 0.12–0.46 s; a 64-block `sync` catch-up 254–280 s → 18 s; byte-identical outputs at corpus scale); **then, owner-approved 2026-09-26, a time budget and a watcher for the measurement harnesses** (`133-performance-cost-and-its-evidence/measurement-budget-handoff-v1.md`, **ACCEPTED 2026-09-26**, `74e7c4ac`, review `measurement-budget-review-v1`: the release-gate profile is 11–13 minutes per run, measured, and 0.47.0 re-baselined on it at 1.916×); **then a defect that review found: every object append read its whole container to learn its length** (a one-file commit peaks at the blob container's size; a first commit of 4,000 × 20 KB files reads 162 GB; RFC 102 `MEASURED and RULED 2026-09-26`; `102-container-based-durability/append-length-without-reading-handoff-v1.md`, live; **the owner ruled it into 0.48.0**, 2026-09-26: *"Yes. The fix should be put in 0.48.0."*; the index tail re-read authorized into it too); **then RFC 160's recurrence guards** (owner-accepted 2026-09-26, *"1. Accepted. 2. 0.48.0"*), before 0.48.0 release prep, **which writes its CHANGELOG section in RFC 161's shape**; 0.47.0 is not re-tagged; **crates.io: 0.47.0 is published together with 0.48.0**, the owner's ruling), implementation in 0.48.0 before RFC 158 Stages B and C if it needs no format change; `seal`, `merge` and **`sync`'s seal-from-accepted** all pay the walk; then the gate plan (held). **Theme 17's prerequisite is now measured**: `seal`'s lineage walk, designed together with 2c's deferred option (i), stands between prikk and any import past a few thousand blocks; scheduling it is the owner's, then **a queued patch's node-addressed operations naming their paths — DELIVERED 2026-09-22** (`061b2f12`, review `queued-patch-paths-review-v1`; paths resolve in prose and JSON, identical before and after `seal`, truncated at the patch shown, bare sealed patch id unchanged), then **naming the unpublished current branch in `branch list` — DELIVERED 2026-09-22** (`945fe817`, review `unpublished-current-branch-listing-review-v1`; one control owed on the closed self-switch, and the round ran ahead of its place because the architect published a handoff as live while saying it was not next — from here a handoff goes live when it is next) (`146-machine-readable-listings/unpublished-current-branch-listing-handoff-v1.md`: one nullable `unpublished_current_branch` field, `branches[]` untouched because a consumer's *membership* test for "published" would otherwise invert silently — stikk letter 017 — plus RFC 147 §2j, `branch switch` naming a route that refuses). From stikk's letter 016: the reported prose-versus-JSON asymmetry does **not** exist — `show <block-id> --format json` already carries every path, measured on the released 0.46.0 binary — but a *queued* patch resolves nothing in either format, and the folded baseline `commit` already derives is the answer `show` declines to compute), then**  — RFC 136 increment 2c, a performance round (the anomaly was not a defect), **plus RFC 158 Stage A** (the import/verify size bound), which closes the measured out-of-memory hole against untrusted input. **0.48.0 "large objects"** — RFC 158 Stages B (streaming) and C (chunk manifests, format 8, explicit upgrade); **0.49.0** — RFC 158 Stage D (explicit reclamation), with RFC 155 then RFC 154 behind it. **Owner-accepted 2026-09-21** (*"Reviewed. All your recommendations are accepted."*), design and order both; each cut still needs the owner's own word. Scope: `merge-evidence`'s second chain walk and the incremental step's missing-text fallback, seeded from the nearest snapshot anchor (direction (a), measured in 2c's design round first; no cache schema change). **0.48.0+ "one repository in one file"** — RFC 155 (its all-or-nothing import gets a design round first), then RFC 154's adoption act. **Unscheduled, in rough order:** `stash`; blame/annotate as JSON (planeter A3); `prikk mv --drop-declaration`; `show` deriving deleted content; RFC 113 history import; the `verify` O(N³) measurement; Windows coverage of the refusal field; the repeated-flag sweep; `merge-evidence` `asserted-by` and queued `status` renames naming every AUTHOR signer (RFC 156 Stage 4 left them one signer); identical operations on both sides of a merge (identical renames or edits) reported as `Conflict` — a whole-policy question (merge-with-renames design review, 2026-09-17); one file-level and one lifecycle rename semantics (checkout's `apply_rename_batch` beside `rename_nodes_checked_batch`). Batch-aware pair proofs, so a one-commit swap merges beside other changes (merge-with-renames review, 2026-09-17). Readers answering `ref namespace is reserved` for a received ref (`worktree-status`, `sync have`, `sync build`) — true but uninformative (refusal sweep stop review, 2026-09-17). `rollback-draft-verify --ref remotes/…` answers with its WAL check before the ref; `init <nonexistent path>` answers `i/o error` (both refusal sweep review, 2026-09-17). **Architect's own, alongside 0.45:** ROADMAP and `rfcs/README.md` currency; record in the patch-exchange design text that `sync accept`'s §8.1 allowance (patches and blobs may survive a failed exchange) was withdrawn in 0.44.0. **Cadence:** a minor when its theme is done and CI is green on every job — never by date. Each cut still needs the owner's word | Product | **SCHEDULED** |
...
|---|---|---|
| ~~**RFC 148**~~ — default key directory | `rfcs/accepted/148-…` | **accepted 2026-09-12**; handoff live; Unix mode rule = refuse unless you say otherwise |
| ~~**RFC 149**~~ | `rfcs/done/149-…` | **closed 2026-09-13** under path B |
| ~~**RFC 150**~~ — `prikk key status` | `rfcs/accepted/150-…` | **accepted 2026-09-12**; handoff live; 0.41 item 0 sequenced behind it |
| ~~**RFC 151**~~ | `rfcs/done/151-…` | **done 2026-09-13**; 0.42.0's theme delivered |
| **RFC 130 §8** — interim growth controls | `rfcs/done/130-…` §8 | file gate + evidence line; control 3 subsumed by RFC 149 |
| ~~**RFC 145**~~ | `rfcs/done/145-…` | **closed 2026-09-13** as ruled; the external architect's side closed with letter 012 |
| RFC 120 §9.4, §9.4a | `rfcs/accepted/120-…` | two rulings; §9.4a stops an error on its third occurrence |
| RFC 133 §6 — memory as a stated requirement | `rfcs/done/133-…` §6a | refuse the unqualified form, state it on the bytes axis |
...
| item | handoff |
|---|---|
| ~~A deletion of a previously edited text file has no stored preimage Blob, so `bundle export`, `sync build` and sealing its inverse all refuse~~ **FIXED 2026-09-16** (`1df0be33`, `5cc9138a`; reviewed `export-of-an-edited-then-deleted-file-review-v2.md`): replay derives the content, exporters carry it, the seal stores it, receivers unchanged — verified by the architect end to end, including a 0.42.0 binary importing a new bundle. **Five follow-ups DONE 2026-09-16** (`4885e61b`; reviewed `derivation-follow-ups-review-v1.md`) — (the refuted merge limit and its control, chain errors swallowed as "nothing derived", `has_object` instead of full reads on the seal path, a false rollback claim in `backup-restore.md`, the apply-layer id-mismatch control): both merge shapes derive and are controlled, chain failures propagate, the index answers presence (no regression on a matched-build measurement), the rollback claim in `backup-restore.md` is corrected, and the mismatch control sits at the apply layer | done |
| ~~A declaration reports what `commit` will do with it; refusal advice names commands that work (RFC 147 §2f, stikk 013)~~ **DONE 2026-09-16** (`ee86aebe`, `e1f7a412`; reviewed `declarations-say-what-commit-does-review-v1.md`): one classifier for commit and status, five resolutions, `content_changed`/`mode_changed`, `refused_declaration_count`, every named route controlled. **Two parity rows (`never-tracked`, `deletion-ignored`) fold into the 0.43.0 prep round** | done |
| ~~**The Windows mutation suite is red and holds the cut**~~ **CLOSED 2026-09-16** (`353c7464`; reviewed `windows-mutation-suite-red-review-v1.md`): paths compared as paths, identical bytes are unchanged where no POSIX mode is observable, a file vanishing mid-checkout still refuses, and a repository-wide sweep for the same test shape found nothing else. **CI green on every job** | done |
| ~~**0.43.0 release commit, take two**~~ **RELEASED 2026-09-16** — `f3860e28`, signed tag `0.43.0`, published to crates.io | done |
...
| ~~G1 — a contradicted rename declaration is refused by `commit` but is not per-path~~ | RFC 147 §2e, §2f | **CLOSED 2026-09-16** (`ee86aebe`, `e1f7a412`): reported per declaration, with commit's own message |
| `prikk mv` refuses while both paths exist, so clearing a contradicted declaration needs a manual deletion first; a `--drop-declaration` verb would remove that step | RFC 147 §2f review, 2026-09-16 | candidate, owner's scheduling; the refusal explains the manual route |
| `show` prints `<unavailable blob …>` for a deletion's preimage in the repository that authored it, while a repository that imported the bundle prints the content — `bundle export` derives it, `show` does not | 0.43.0 prep report §3.4, 2026-09-16 | pre-existing, not 0.43.0's; candidate |
| `rfcs/README.md`'s RFC index is stale — rows 133, 145 and 149 (among others) link into `proposed/` though those files have moved, and no gate checks the table | found at the 0.43.0 cut, 2026-09-16 | one currency round; say whether a gate should bind it |
| G2 — baseline symlink nodes skipped by `worktree-status` | RFC 147 §2e | out of scope until symlink authoring is |
| `sync build`'s missing-blob refusal does not name an unwalkable block chain, while `bundle export`'s does (`artifact.rs` uses plain `read_required`) | DC-78 follow-ups review, 2026-09-16 | cosmetic: the shape is one no writer produces |
...
| "ref is not published" / "received ref does not exist" answer three different classes across commands (`refs.rs:148` `Integrity`, `rollback/draft.rs:216` `InvalidName`, merge's `Integrity`) | DC-75 two-edits inventory, 2026-09-15 | **HANDED OFF 2026-09-17** (0.45.0 item 3): `132-error-taxonomy-structure/absent-and-received-ref-refusals-handoff-v1.md`; measured also `log`/`worktree-status` answering a state for an absent ref and `--snapshot-plan` naming a route that refuses |
| **The default key id collides across installations** — **HANDED OFF 2026-09-17** (0.45.0 item 1, `135-first-run-entrance-and-configuration/distinct-default-key-ids-handoff-v1.md`; authors collide too: a default installation refuses another's bundle). `setup` adopts every new maintainer key as `maintainer` and ignores `PRIKK_MAINTAINER_KEY_ID` (measured 2026-09-15). A repository adopts one key per id, so two default installations cannot trust each other, and cross-repository `sync accept` fails on the claim's signature. Affects every multi-party use: planeter's forge, and sync between two people | planeter letters, 2026-09-15 | a design question (per-key default ids, or trust keyed by key rather than id): own RFC, before a forge relies on sync |
| ~~**The backup guide's restore claim is false** — *"Turning `remotes/heads/main` into your own working `heads/main` is an ordinary `merge`"*: measured, a fresh repository cannot merge into an unpublished `heads/main` (and without keys, cannot sign at all). `branch create --from remotes/…` and `checkout --patch-materialize --ref remotes/…` refuse too~~ | planeter letters, 2026-09-15 | **CLOSED 2026-09-16** in RFC 136 increment 3 (`fef13536`): rewritten from measurement as *What a restore gives, and what it does not* |
| **A repository-complete, relayable artifact** — RFC 114 §5.2 (owner-ruled) requires a supported, tested carry-forward before format 7 ships, and says a one-ref bundle landing at `remotes/` is not it; RFC 116 left it *"its own RFC"*. A keyless forge needs the same capability to serve maintainer-signed history onward (a received ref cannot be re-exported today, measured) | RFC 114 §5.2, RFC 116 §7, planeter letters 2026-09-15 | own RFC; planeter offered to co-author; scheduling is the owner's |
| ~~**The sync guide's id claim is false** — *"A sender's block and the receiver's sealed block for the same patches are different objects … expect the ids to differ"*: measured 2026-09-15, a receiver sealing the same patches in the same grouping produces **identical** block and RefState ids (an id is the payload's hash; signatures are outside it, `envelope.rs:51-57`). Only the signed envelopes differ. RFC 136 §10.2 already relies on the same determinism~~ | planeter letters, 2026-09-15 | **CLOSED 2026-09-16** in RFC 136 increment 3 (`fef13536`): ids excluding signatures, and one signed copy per id, stated as measured |
| **One repository holds one envelope per id, so the same history under a second signer is refused as damage** — an object id excludes signatures (`envelope.rs:51-57`), and a same-id write with different envelope bytes is refused (`foundation/index.rs:443-469`). Measured 2026-09-15: importing a maintainer's bundle into a repository that had re-sealed the same history under its own key fails with `integrity error: existing container record … differs from candidate`; `verify` stays clean. The refusal is safe (no signature is swapped), but its class calls a legitimate state damage, and a multi-signer design (a forge, a mirror) cannot hold both | planeter letters, 2026-09-15 | class per RFC 132 now; whether one id may carry several signature sets belongs to the repository-complete artifact RFC |
...
| `prikk config` — durable non-secret settings — **SCHEDULED 0.47.0** as RFC 158 Stage A's home for the large-object bound (owner-accepted 2026-09-21) | RFC 135 §9.1, **§9.1a (2026-09-12)** | Trigger: *a first real adopter*, restated concretely in §9.1a: after RFC 148 defaults the key ids and moves the seeds to a directory, **the only user-facing env configuration left is the active-patch threshold pair, `PRIKK_ACTIVE_PATCH_WARN` / `PRIKK_ACTIVE_PATCH_LIMIT`** — one mechanism is not a beneficiary. *(Not so, found 2026-09-23: six DC-86 limit variables — `PRIKK_BUNDLE_MAX_BYTES`/`_OBJECTS`, `PRIKK_EXCHANGE_MAX_BYTES`/`_OBJECTS`, `PRIKK_SYNC_SUMMARY_MAX_BYTES`/`_REFS` — exist and are undocumented; Stage A documents them.)* `--allow-no-audit` stays a loud per-seal decision by design; `--ref` is the branch-switch question. **Re-opens when a second undefaultable per-user/per-repo setting appears or an adopter asks.** **Format candidate recorded in §9.1a so it is not forgotten: `app-json-settings` 2.7.0 (`serde`+`serde_json`, MSRV 1.85), to be weighed against a hand-built reader as an `ALLOWED_THIRD_PARTY` decision per RFC 135 §4 when the trigger fires** |
...
| **Replay has no per-entry "unavailable" state**: a missing or non-recomputing blob fails the whole read, so `checkout --patch-plan`, `tree`, `bundle export` and `merge` can report damage only by refusing entirely. Measured 2026-09-21 (tree Stage 1 report §5.1): a history naming an unstored blob cannot be sealed at all, and post-seal damage surfaces as the index's own `Integrity` | RFC 157 §5a, which amended the architect's unreachable `unavailable` clause | a later increment would give replay an explicit per-entry state with its own controls, shared by every reader. `show` keeps `<unavailable blob …>`, which is a different question: it renders one patch's operand and never replays a tree |
| **Large objects (binary assets): no bound, no streaming, no delta, no compression, no reclamation** — **measured 2026-09-21 on the released 0.45.0 asset**: a 32 MiB file costs 33 MB stored; a **one-byte change stores the whole file again** (65 MB); an identical copy costs nothing (content addressing dedupes whole files); a bundle carries every version; **peak memory is ~5× the file at `commit`** (644 MB for 128 MiB), ~4× at `verify`, ~2× at `seal`/`checkout`/`show`. Nothing bounds an object's size anywhere, so an untrusted bundle naming a huge blob is an out-of-memory kill rather than a refusal. **Corrected 2026-09-23:** DC-86 does bound every incoming artifact (bundle and exchange artifact 256 MiB, summary 16 MiB, have-list 64 MiB), but all six CLI entry points read the whole file before the store compares its length — a sparse 1 GiB file peaks at 1,051,060 KB in 0.46.0's `bundle verify`, then refuses; the out-of-memory conclusion stands, the cause is the read order (RFC 158 Status) | measured by the architect 2026-09-21, on the owner's question | **RFC 158** (`rfcs/accepted/158-large-objects.md`): Stage A bound, Stage B streaming, Stage C chunk manifests as format 8, Stage D reclamation. **Owner accepted 2026-09-21**: the bound lives in **`prikk config`**, which Stage A builds; the verb is **`prikk reclaim`**; the schedule is RFC 158 §8.3 |
...
before the tag**, `Release` green (5 jobs) with **16 assets**, not a draft. **All eight crates published to crates.io
in topological order and confirmed live against the sparse index, none yanked; `cargo install prikk
--version 0.39.0` from the registry builds, runs, and answers all three listing schemas.** Packaged content
was byte-identical between the tag and the publishing commit (only `ROADMAP.md` differed).

**One defect found on the published binary, small and mine:** `prikk --help` does not advertise
`--format json` on `log`, `branch` or `tag` — the RFC 146 handoff forbade prose changes and the synopsis
is prose. Behaviour is correct; the help line lags. RFC 146 §8e; handoff issued.
