...
| **0.42.0 release position (2026-09-13, evening)** | | | **Theme delivered**: RFC 151 in full, plus the lifecycle gate, the CI policy job with required steps, `size-check`, the layer rule, the derived crate order. **Two small stikk rounds are handed off and user-visible** (RFC 147 §3f, RFC 142 §7a); the architect recommends cutting 0.42.0 once they land, via the release-prep handoff. |
...
| **13** | **RFC 142 §7a / RFC 140 — a queued patch's message travels in `status`; `show` renders a queued patch from the WAL; an unknown id is a precondition, not an integrity error** (stikk letter 011) | Product | **Handed off 2026-09-13** — `142-content-surface/queued-patches-handoff-v1.md`; one round, after RFC 151's follow-up |
| **12** | **RFC 147 §3f — `unsupported-path` entries say `authored` while `commit` refuses them; their `path` is absolute** (stikk letter 010) | Product | **Handed off 2026-09-13** — `147-…/unrepresentable-names-handoff-v1.md`; small; the same one-classifier fix as Case A |
