...
- [`143-content-at-a-point.md`](rfcs/proposed/143-content-at-a-point.md) — RFC 143, Content at a point: exposing the replay result we already compute (**opened 2026-09-08** at the stikk project's request, on the owner's instruction). A read-only `--format json` for `checkout --patch-plan`, **path-scoped so cost is bounded by an argument that already exists**. **Commissions no computation** — verified at source: `ReplayManifestEntry` carries `bytes: Vec<u8>` and `total_content_bytes()` sums their lengths, so the plan materializes the content and reports its size instead. **`prikk diff` stays refused** (RFC 142 §5/§7): this owns no comparison semantics, and the requesting project takes the comparison and the responsibility for it. **§3 corrects the requester's own consistency argument** — nine read commands lack `--format json`, not one. **§6 binds the replay's partial coverage into a machine-readable field**, the third instance of RFC 140 §7b after RFC 142 §6b. **Not accepted; no implementation may start**)
