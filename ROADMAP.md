...
- `159-sealing-without-rewalking-the-lineage.md` — `seal`, `merge` and `sync`'s seal-from-accepted each walk the whole lineage per block, so building or importing a deep history is quadratic (measured in release to depth 1,024); proposed 2026-09-25, design round scheduled first after the 0.47.0 cut; history import's prerequisite.
