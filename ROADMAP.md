...
| **1** | **RFC 144 — declared-move capture and rename authoring** | Product | **ACCEPTED 2026-09-09; UNGATED** — §4k.1, its only remaining gate, closed at `bb2e932d`. **Needs a design section before a handoff**: RFC 144 rules the policy (declaration-only, hint not mandate) but says "`prikk mv`, **or equivalent capture**" — the surface is deliberately unspecified, the same state `bundle preview` was in at §4f before §4m. `prikk mv` plus authoring, the non-authoring commit-time hint (§4h.5), the thirteenth conflict witness for both-sides-rename (§4i.2), and §4i.1's honesty invariant enforced **structurally** — rename and asserting signer as one value in the read types, so no future surface can drop the signer |
