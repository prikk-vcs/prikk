...
| **3** | **RFC 144 — the bundle-impact preview** | Product | **DESIGNED and HANDED OFF 2026-09-09** (§4m; `rfcs/handoffs/144-two-point-comparison/bundle-impact-preview-handoff-v1.md`). **Reading the existing surface narrowed the claim**: `merge-plan` is already read-only but takes refs, so it can only answer *after* `bundle import` has written the bundle's objects — the act the preview must precede is **admitting foreign objects into the store**, not `merge`. `prikk bundle preview --input <file>`, writes nothing, proven by a control. **ACCEPTED 2026-09-09.** *What would accepting this bundle do to my repository* — a **safety surface consulted before an irreversible act**, and the one comparison worth owning first-party (§4f). **Separable: depends on neither the rename decision nor increment 1** (§4g.3), so its position is free |
