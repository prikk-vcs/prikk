...
**Status.** **ACCEPTED 2026-09-13** — the owner ruled §4.3–§4.5 by accepting the architect's recommendations (below); the gate on any per-source RFC is discharged. What follows is design: the import contract on prikk's side (foundations), then Git, then Subversion, then CVS (§5) — the architect writes the foundations design when the owner schedules it; brygge decodes. Earlier: **Proposed.** Originated by the project owner 2026-08-19: *"I would like to offer migration
...
**RULED 2026-09-13 — the owner accepted the architect's recommendations verbatim** (`.git-exclude/reviewed/owner-blocked-items-review-2026-09-13.md`): §4.3 — the importer signs the *import declaration* under its own key, source authorship carried as recorded text, never asserted; §4.4 — only an adopted maintainer seals imported history, and the importer's key must be one; §4.5 — refuse, never approximate: submodules, symlinks, non-UTF-8/backslash paths, replace refs and grafts, octopus merges in the first cut; commit signatures carried as text; every refusal reported before any object is written.
