...
| 143 | [Content at a point](./proposed/143-content-at-a-point.md) | Unscheduled. **Opened 2026-09-08 at the stikk project's request**, on the owner's instruction. A read-only `--format json` for `checkout --patch-plan`, path-scoped. **Commissions no computation**: `ReplayManifestEntry` already carries `bytes: Vec<u8>` and the plan reports their summed length instead. **`prikk diff` stays refused** (RFC 142 §5/§7) — this owns no comparison semantics; the consumer takes the comparison and the responsibility for it |
