...
| `git show <rev>:<path>` | [`prikk cat --path <p> [--ref <ref\|block-id>]`](../guide/tree-and-cat.md) | **Bytes, bounded.** Writes the file's content at a point to stdout or `--output`; `--max-bytes` refuses anything larger, writing nothing. Binary content refuses a terminal and names `--output`. `--format json` (`path-content-v1`) reports the metadata with no bytes. |
| `git diff A B` | [`prikk diff --from <ref\|block-id> --to <ref\|block-id>`](../guide/diff.md) | Compares the state at any two refs or block ids and prints unified hunks (3 lines of context) for each text change; a rename is shown only when history *declares* one (`prikk mv`), never guessed from similarity; a binary file shows ids and sizes, and `prikk cat` reads either side. `--format json` is `diff-report-v1`. |
| `git diff` (worktree against `HEAD`) | [`prikk diff`](../guide/diff.md) | **What `commit` would author, with content.** Bare `prikk diff` compares the current branch's tip (and any queued commits) with the worktree; `prikk diff --from <point>` compares that point. There is no index, so a new file is an `added` entry with its content — there is no "untracked" list — and ignored paths never appear. `commit` then `diff` is empty again. |
...
| `git config` | [`prikk config get\|set\|unset\|list`](commands.md#size-bounds-on-incoming-artifacts) | **Far narrower.** One repository-scoped key exists today, `incoming.max-object-bytes` (RFC 158 Stage A), at `.prikk/config` — never global, never per-user. Signing keys are still read from a per-user key directory by default, with `PRIKK_*_SEED_FILE` / `PRIKK_*_KEY_ID` environment variables overriding it — see [Security and Signing Setup](../guide/security-setup.md#current-key-inputs). |
