...
| `git diff A B` | `prikk diff --from <ref\|block-id> --to <ref\|block-id>` | **Two points only, so far.** Compares the state at any two refs or block ids and prints unified hunks (3 lines of context) for each text change; a rename is shown only when history *declares* one (`prikk mv`), never guessed from similarity; a binary file shows ids and sizes, and `prikk cat` reads either side. `--format json` is `diff-report-v1`. |
| `git diff` (worktree against `HEAD`) | — | **Not built yet.** Comparing the worktree with the current branch is the next increment of RFC 153. Until then [`prikk worktree-status`](../guide/worktree-status.md) names the changed paths against the baseline, without content. |
