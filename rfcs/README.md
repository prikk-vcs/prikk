...
| 147 | [Refusal Visibility](./proposed/147-refusal-visibility.md) | Unscheduled. Opened 2026-09-10 on two stikk reports, both reproduced first. A read surface must not describe as ordinary a path a write will refuse: a symlink reads `untracked` while `unsupported paths: 0`, and a tag created at a block is declined by checkout outside checkout's own vocabulary. **Owner rulings required** |
