...
| **1** | **RFC 148 — a default key directory, two `0600` files, env variables as overrides only** | Product | **Awaits acceptance — amended twice 2026-09-12 on the owner's questions**: first the environment channel went; then *"requiring paths depends on an individual user"* replaced it with a per-platform default (`$XDG_CONFIG_HOME/prikk`, `%APPDATA%\prikk`). A fresh shell needs **zero** exports. `app-json-settings` evaluated on merit (serde + serde_json): not needed for a directory and two files; the candidate when `prikk config` opens. Breaking-once CLI change: `PRIKK_*_SEED` becomes a one-release loud refusal, `PRIKK_*_SEED_FILE` the only channel. Migration measured: 3 source files through one reader, 36 test files through one helper, 8 docs pages, `ci.yml`, `tools/corpus`, 5 sites in stikk's seam. Ships with the item below — same command, same page |
...
|---|---|---|
| **RFC 148** — default key directory, overrides only (amended twice on the owner's questions) | `rfcs/proposed/148-…` | accept as amended; Unix mode rule; ship as 0.40.0's headline |
