...
- [`151-the-current-branch.md`](rfcs/proposed/151-the-current-branch.md) — RFC 151, The current branch and `branch switch` (opened 2026-09-13 on the owner's instruction to review the open directions). A local pointer as the default for `--ref`; `switch` from a clean worktree only, with bounded deletion. **Awaits the owner's three yes/no answers (§3).**
...
| **11** | **CI runs none of the policy gates** (`check`, `boundary-check`, `reference-check`, `size-check` are local-only; found at the RFC 141 §7a review) | CI | RFC 141 increment 4's first task, with the `check-release-evidence <document>` caller; owner schedules |
...
| **RFC 151 — the current branch** | `rfcs/proposed/151-…` §3 | **three yes/no answers**; architect recommends yes to all; **proposed as 0.42.0's theme** |
| **RFC 130 §8** — interim growth controls | `rfcs/done/130-…` §8 | file gate + evidence line; control 3 subsumed by RFC 149 |
| **RFC 145** — browse-view shape (D over the CLI) | `rfcs/proposed/145-…` §9 | **accept as ruled and close** — nothing to implement; architect's recommendation restated 2026-09-13 |
