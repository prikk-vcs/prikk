...
| **2** | **`prikk setup` refuses on an existing repository before touching anything** | Product | **Live** — `135-…/setup-existing-repository-handoff-v1.md`. Direction approved by the owner |
| **3** | **Object-container writes protected against each other** | Product | **Handed off** — `102-container-based-durability/object-container-write-locking-handoff-v1.md`. Measure every object-appending path's locks first, then choose the mechanism at a checkpoint: the page's own two options are `ActiveLock` across `import_bundle`'s writes or a dedicated object-container lock, and which is honest depends on whether `commit` vs `import` can race too |
| **4** | RFC 146 §8e — three listing synopses advertise `[--format json]` | Product | **Live** — `146-…/help-synopsis-handoff-v1.md` |
