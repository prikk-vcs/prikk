...
|---|---|---|
| **A** `rollback_{draft,preview,verify}` → `rollback/` | 4 rewrites, no allowlist change, no cycle gained or lost, gate green | **DONE 2026-09-12.** `rollback::{draft,preview,verify}`; 2 rewrites (`verify.rs`, `history.rs`) plus 2 inside the family; nodes 128 → 129, edges 483 → 483, hubs unchanged (5), `subtree_cycles` unchanged (13), gate `valid`; root-export names unchanged (267, the 2 additions since 0.39.0 are RFC 102's repair round). **§2.2a's refusal of this same grouping is superseded, not overturned** — it measured a real `verify <-> rollback` cycle on the pre-qualified-naming graph, where a group collapsed to one node whose edges were its members' union; qualified naming removed that collapse |
