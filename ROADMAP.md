...
- [`150-signing-readiness.md`](rfcs/proposed/150-signing-readiness.md) — RFC 150, `prikk key status` (opened 2026-09-12 on stikk's letter 007, received before 0.40.0 published). Per-role signing readiness from the same function `commit`/`seal` use; `binding` answers whether the displayed key id and the seed agree. **Sequencing rule: the retired-variable detection removal does not ship before it.** Awaits acceptance; handoff drafted and held.
...
| 0b | `setup`'s help synopsis says "print the exports" — true only for user-named seed paths since RFC 148 | Product | **Handed off** — `148-…/setup-synopsis-handoff-v1.md`; found at the 0.40.0 readiness check, a description not a flag, did not hold the cut |
| **0c** | **RFC 150 `prikk key status`** — signing readiness for front-ends, per stikk letter 007 | Product | **Proposed 2026-09-12, awaits acceptance**; handoff held at `150-signing-readiness/key-status-handoff-v1.md`. **Item 0's removal waits for this** |
...
| **8** | RFC 131 §6e — grouping census | Product | **Live**; its result feeds RFC 149's family boundaries rather than a separate grouping |
...
| **RFC 150** — `prikk key status`, signing readiness | `rfcs/proposed/150-…` | **accept or not**; stikk is waiting on it and 0.41's retired-variable removal is sequenced behind it |
