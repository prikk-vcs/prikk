...
| **1** | **RFC 131 §6d.3 — re-audit eligibility, comment-aware** | Product | **RULED 2026-09-10; handed off** at `rfcs/handoffs/131-module-grouping-and-visibility-scoping/eligibility-reaudit-handoff-v1.md`. **The yield was understated fivefold**: 5 of 7 items eligible, not 1 — `author_worktree_patch`'s two cited callers are both **comments**. Cause is method: a raw grep cannot tell a call from a prose mention, and **the gate's own scanner blanks comments before scanning for exactly this reason**. 71% not 14%, which strengthens the case for grouping |
