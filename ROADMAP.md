...
- [`144-two-point-comparison.md`](rfcs/proposed/144-two-point-comparison.md) — RFC 144, Two-point comparison: reopening `prikk diff` (**opened 2026-09-08** on the owner's instruction). **Reopened because the refusal's own premise expired** — RFC 142 §5 refused on cost (two unbuilt replays) and RFC 143 shipped and measured endpoint reconstruction. **§2b records what does NOT transfer**: RFC 142 §6's refusal to synthesize line hunks does not reach a two-point delta, which has no spans. **§2c/§4 carry the real question** — `commit` authors no renames, so any comparison reports them as delete+create, and the decision may belong to the patch model rather than to a read surface. **Not accepted; no implementation may start**)
