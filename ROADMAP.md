...
| **1** | **RFC 148 — a default key directory, two `0600` files, env variables as overrides only** | Product | **One round (`ea0b16f2`+`a0f7b0a2` returned 2026-09-12).** Core measured right: bare `setup` → sealed commit with zero `PRIKK_*`, `0700`/`0600`, every refusal fires. **Returned because `setup` on a second project initialises the repository and then refuses on the existing seed** — RFC 148 §2c now rules reuse, every check before `init`; plus the RFC 102 race control flaked in the architect's gate run (assertion to be made robust) and the removal comment says 0.42.0 for a 0.41.0 window |
