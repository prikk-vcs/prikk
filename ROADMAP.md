...
| **1** | **RFC 147 §3d — `branch create --from` a tag resolves; `tag create --target` a tag refuses; one `current_target_block`** | Product | **RULED and handed off 2026-09-12** at `rfcs/handoffs/147-refusal-visibility/tag-target-semantics-handoff-v1.md`, from the round's own census. `--from <ref>` means "at the block this ref names" — resolve. `--target tags/<t>` is refused as a **precondition naming both accepted forms**, not dereferenced: a tag-of-a-tag is outside the one-hop model and silent collapse would make two requests indistinguishable in history. The two same-named resolvers consolidate to one, direction chosen against the coupling gate |
