...
| ~~**3 → FIRST**~~ | ~~Object-container writes protected against each other~~ | Product | **DELIVERED 2026-09-12 (`e2c86373`), held unpushed with item 3b until its one round lands.** Leaf lock in the surface-layer wrapper (the `foundation ↔ lock` cycle refused — RFC 102's dated section). **Architect's stress on the fixed binary: 4 of 4 clean** where the old binary damaged 4 of 4; every refusal a lock conflict |
| **3b** | **`prikk doctor --repair-index`** | Product | **One round (`2ec9aa40` returned)** — repaired all four really-damaged repositories and a garbage index, **but holds no lock: forty repairs beside forty `tag create` left `verify` failing.** v3 handoff: acquire the object-store lock from the surface layer, non_exhaustive on the report struct, two stale doc lines |
