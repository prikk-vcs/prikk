...
| **2** | **RFC 144 increment 3 pieces 2-4** — commit-time hint, thirteenth witness, structural honesty invariant | Product | **DESIGNED 2026-09-09 (§4o.4-§4o.6), not yet handed off.** Deliberately split from piece 1: the hint authors nothing and must never become a default; the thirteenth is one-node-two-destinations (`SamePathCreate` stays, relabelling deferred); **piece 4 changes read types across `show`, `bundle preview` and `checkout --patch-plan`**, so it wants its own round |
