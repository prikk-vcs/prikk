...
- [`147-refusal-visibility.md`](rfcs/proposed/147-refusal-visibility.md) — RFC 147, what the read surface promises about what a write will refuse (opened 2026-09-10 on stikk letter 006 §4/§6, **both reproduced against the shipped 0.38.0 binary before opening**). **Case A:** a symlink is reported `untracked` in prose *and* in `--format json` while `unsupported paths: 0`, and then `commit` refuses the **whole** commit — one such file blocks every commit in the repository. Root cause at source: `UnsupportedPath` is produced in exactly one place, the `Err` arm of `RepoPath::parse`, so it means **"unrepresentable name"**, not **"a write will refuse this"**. **Widening that shipped `kind` string would silently redefine a machine-branchable value**, which RFC 140 §7b and RFC 142 §6b exist to prevent — four options weighed, unruled. **Case B:** `tag create --target <block>` succeeds and `checkout --ref tags/<name>` then fails `object type mismatch`; a control stikk did not run shows **checkout already owns a machine-branchable vocabulary for this** (`materialization: unpublished-ref`, exit 0) that tags do not use — so the finding is that they fail *outside* the vocabulary, not that they fail. **RULED: neither of the two `Integrity`-for-`Precondition` misclassifications found today fires an RFC 132 re-open trigger** — choosing the wrong existing variant is a defect against the taxonomy, not evidence it needs more variants. **Both cases RULED by the architect 2026-09-12 (§2e, §3b) on the owner's instruction — acceptance is the owner's; handoffs written and held until `accepted/`.** **Case B reversed the architect's lean**: `refs::resolve_ref_tip_block` already exists, consolidated after three re-derivations, and `bundle` uses it while `checkout.rs:138` and `history.rs:277` never did — ruled dereference through it, not refusal at `tag create`, which would have broken the command that works. **Case A: a per-entry `authoring: refused` field from one classifier shared with `node_authoring`**, additive within `-v1` on the `declarations` precedent — the tracked-symlink case is *both* `modified` and refused, so no `kind` can carry it; `Unsupported*` `AuthorError`s → `Precondition` in the same round
