...
| **0.40.0 — "setup and keys survive a reboot"** | | | **The next cut's theme.** Every item below is small; together they make the entrance work the day after it is used |
| **1** | **RFC 148 — one seed channel, a `0600` file; the environment channel removed** | Product | **Awaits acceptance (amended 2026-09-12 on the owner's question).** Breaking-once CLI change: `PRIKK_*_SEED` becomes a one-release loud refusal, `PRIKK_*_SEED_FILE` the only channel. Migration measured: 3 source files through one reader, 36 test files through one helper, 8 docs pages, `ci.yml`, `tools/corpus`, 5 sites in stikk's seam. Ships with the item below — same command, same page |
...
| **3** | **`import_bundle` holds `ActiveLock` across its object writes** | Product | **Moved up on correctness grounds** from the inventory's band C: a documented write-path locking gap (`concurrency-locking.md:202`). Needs a handoff; small |
| **4** | RFC 146 §8e — three listing synopses advertise `[--format json]` | Product | **Live** — `146-…/help-synopsis-handoff-v1.md` |
| **5** | `checkout --snapshot-plan`'s by-design missing snapshot → `Precondition`; `security-setup.md:22`'s stale AUTHOR-trust sentence | Product / Docs | Two one-line items; fold into RFC 148's docs sweep or a per-site round |
...
| **7** | **RFC 149 — layer `prikk-store`: core stays, 26 surfaces move above it** | Product | **Awaits acceptance.** Measured cut: zero core→surface edges, zero through 264 root re-exports. Sequenced after step 0 and a public-API census; one move commit per family |
...
|---|---|---|
| **RFC 148** — one seed channel, environment channel removed (amended on the owner's question) | `rfcs/proposed/148-…` | accept as amended; Unix mode rule; ship as 0.40.0's headline |
| **RFC 149** — layer `prikk-store` above its cyclic core | `rfcs/proposed/149-…` | accept the direction; name the surfaces crate; gate before or alongside |
